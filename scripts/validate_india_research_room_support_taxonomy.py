...
def test_buy_india_support_allowed():
    ...
    card = Card('印度奧援', 'support', {'money': 0})
    g.purchase_area = [card]
    g.static_purchase_supply['印度奧援'] = 1
    result = g.buy_card(0)
...
def main():
    ...
    (RECORD_DIR / 'INDIA_RESEARCH_ROOM_SUPPORT_TAXONOMY_VALIDATION.json').write_text(json.dumps(payload, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    ...
    (RECORD_DIR / 'INDIA_RESEARCH_ROOM_SUPPORT_TAXONOMY_VALIDATION.md').write_text('\n'.join(lines) + '\n', encoding='utf-8')
