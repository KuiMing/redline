...
def _make_app(provider):
    routes = NpcRedDissolveTestRoutes(provider)
    app = FastAPI()
...
def test_npc_red_dissolve_default_state_and_lobby_registration():
    runtime = _runtime()
    result = NpcRedDissolveTestRoutes(
        lambda: runtime
    ...
    assert [event["name"] for event in game.event_deck.discard_pile] == ["全國人大召開"]
...
def test_main_npc_red_dissolve_uses_rebound_runtime_and_callable(monkeypatch):
    manager = FakeManager()
    lobby = {}
    hosts = {}
    factions = {}
    ...
    response = TestClient(main.app).post(
        "/test/setup-national-people-congress-red-dissolve-proof", json={}
    )
    ...
    direct = main.test_setup_national_people_congress_red_dissolve_proof({})
    assert direct["game_id"] != game_id
    assert direct["game_id"] in manager.games
    assert callable(main.test_setup_national_people_congress_red_dissolve_proof)


def test_npc_red_dissolve_first_two_uuids_are_the_players():
    runtime = _runtime()
    result = NpcRedDissolveTestRoutes(
        lambda: runtime
    ).test_setup_national_people_congress_red_dissolve_proof({})
...
def test_npc_red_dissolve_http_openapi_and_route_order():
    runtime = _runtime()
    ...
    assert operation["operationId"].startswith(
        "test_setup_national_people_congress_red_dissolve_proof_"
    )
