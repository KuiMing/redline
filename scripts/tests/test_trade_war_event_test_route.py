...
def _make_app(provider):
    routes = TradeWarEventTestRoutes(provider)
    app = FastAPI()
...
def test_trade_war_event_default_state_and_lobby_registration():
    runtime = _runtime()
    result = TradeWarEventTestRoutes(
        lambda: runtime
    ...
    assert result["event_name"] == "貿易戰加劇"
    assert result["purchase_index"] == 6
    ...
    assert viewer.resources == {"money": 4, "propaganda": 0}
    assert [card.name for card in viewer.hand] == ["追隨者"]
    assert [card.name for card in viewer.deck.draw_pile] == ["原牌庫頂下方"]
    assert [card.name for card in viewer.deck.discard_pile] == ["舊棄牌"]
    ...
    assert game.purchase_area[-1].name == "擴大戰果"
...
def test_main_trade_war_event_uses_rebound_runtime_and_callable(monkeypatch):
    manager = FakeManager()
    lobby = {}
    hosts = {}
    factions = {}
    ...
    response = TestClient(main.app).post("/test/setup-trade-war-event-proof", json={})
    ...
    direct = main.test_setup_trade_war_event_proof({})
    assert direct["game_id"] != game_id
    assert direct["game_id"] in manager.games
    assert callable(main.test_setup_trade_war_event_proof)


def test_trade_war_event_first_two_uuids_are_the_players():
    runtime = _runtime()
    result = TradeWarEventTestRoutes(
        lambda: runtime
    ).test_setup_trade_war_event_proof({})
...
def test_trade_war_event_http_openapi_and_route_order():
    runtime = _runtime()
    app = _make_app(lambda: runtime)
    client = TestClient(app)
    response = client.post("/test/setup-trade-war-event-proof", json={})
    assert response.status_code == 200
    ...
    assert client.post("/test/setup-trade-war-event-proof").status_code == 422
    list_response = client.post("/test/setup-trade-war-event-proof", json=[])
    assert list_response.status_code == (
        422 if int(pydantic.VERSION.split(".")[0]) >= 2 else 200
    )

    operation = app.openapi()["paths"]["/test/setup-trade-war-event-proof"]["post"]
    assert operation["summary"] == "Test Setup Trade War Event Proof"
    assert operation["operationId"].startswith("test_setup_trade_war_event_proof_")
    ...
    matching = [
        route
        for route in routes
        if getattr(route, "path", None) == "/test/setup-trade-war-event-proof"
    ]
    assert len(matching) == 1
    assert getattr(matching[0], "methods", None) == {"POST"}
    paths = [getattr(route, "path", None) for route in routes]
    index = paths.index("/test/setup-trade-war-event-proof")
    assert paths[index - 1] == "/test/setup-national-people-congress-inner-build-proof"
