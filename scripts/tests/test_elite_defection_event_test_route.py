...
def _make_app(provider):
    routes = EliteDefectionEventTestRoutes(provider)
    app = FastAPI()
...
def test_elite_defection_event_default_state_and_lobby_registration():
    runtime = _runtime()
    result = EliteDefectionEventTestRoutes(
        lambda: runtime
    ...
    assert result["event_name"] == "紅軍權貴出逃"
    assert result["moves"] == [
    ...
    assert viewer.moves_left == 3
    assert viewer.resources == {"money": 0, "propaganda": 0}
    assert [card.name for card in viewer.hand] == ["手牌移除目標", "手牌保留"]
    assert [card.name for card in viewer.deck.draw_pile] == ["牌庫保留"]
    assert [card.name for card in viewer.deck.discard_pile] == ["棄牌移除目標"]
...
def test_main_elite_defection_event_uses_rebound_runtime_and_callable(monkeypatch):
    manager = FakeManager()
    lobby = {}
    hosts = {}
    factions = {}
    ...
    response = TestClient(main.app).post(
        "/test/setup-elite-defection-event-proof", json={}
    )
    ...
    direct = main.test_setup_elite_defection_event_proof({})
    assert direct["game_id"] != game_id
    assert direct["game_id"] in manager.games
    assert callable(main.test_setup_elite_defection_event_proof)


def test_elite_defection_event_first_two_uuids_are_the_players():
    runtime = _runtime()
    result = EliteDefectionEventTestRoutes(
        lambda: runtime
    ).test_setup_elite_defection_event_proof({})
...
def test_elite_defection_event_http_openapi_and_route_order():
    runtime = _runtime()
    app = _make_app(lambda: runtime)
    client = TestClient(app)
    response = client.post("/test/setup-elite-defection-event-proof", json={})
    assert response.status_code == 200
    ...
    assert client.post("/test/setup-elite-defection-event-proof").status_code == 422
    list_response = client.post("/test/setup-elite-defection-event-proof", json=[])
    assert list_response.status_code == (
        422 if int(pydantic.VERSION.split(".")[0]) >= 2 else 200
    )

    operation = app.openapi()["paths"]["/test/setup-elite-defection-event-proof"][
        "post"
    ]
    assert operation["summary"] == "Test Setup Elite Defection Event Proof"
    assert operation["operationId"].startswith(
        "test_setup_elite_defection_event_proof_"
    )
    ...
    matching = [
        route
        for route in routes
        if getattr(route, "path", None) == "/test/setup-elite-defection-event-proof"
    ]
    assert len(matching) == 1
    assert getattr(matching[0], "methods", None) == {"POST"}
    paths = [getattr(route, "path", None) for route in routes]
    index = paths.index("/test/setup-elite-defection-event-proof")
