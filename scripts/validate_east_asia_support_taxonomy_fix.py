...
def _new_game(org_towns, variant_index=0):
    ...
    player.base = '北京'
    # 北京維持紅軍唯一合法根據地；上海另提供 deterministic 的牆內建立起點。
    # 兩者 ruler=紅軍，不會影響東洋奧援自身區域或 II 級印刷配對地區的領先判定。
    player.organizations = {**{town: 1 for town in org_towns}, '北京': 1, '上海': 1}
    red.faction_id = 'liberals'
    red.base = '巴黎'
    red.organizations = {'巴黎': 1}
    red.hand = []
