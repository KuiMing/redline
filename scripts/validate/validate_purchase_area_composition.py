...
def main():
    ...
    report = {
        ...
        'deviation_1_market_modes': (
            ...
            '2026-07-18 使用者裁決：先不改；日後若要做規則書 100 張模式（間諜/組織/整肅全數＋隨機18奧援＋隨機35其它），'
            '可取代 sample_53 或做成 lobby 第三個選項。'
        ),
        ...
        'deviation_4_deck_refill': (
            '規則書「牌庫用盡時從剩餘行動卡任取一疊補上」；實作為重建整份 initial deck（近似，已買走的卡會再次出現）。'
            '2026-07-18 使用者裁決：先不改；日後若要做，可把開局未抽進牌庫的卡記成剩卡池、用盡時從中補、剩卡用完即不再補。'
        ),
