...
def run_checks():
    ...
    # Tibet red suppression: Red Army may discard any number of hand cards, then builds the same number near a Tibet organization.
    game, actor, red = make_game('tibet')
    tibet_town = '列城'
    actor.organizations = {tibet_town: 1}
    red.hand = [
        Card('紅軍棄牌測試一', 'money', {'money': 1}),
        Card('紅軍棄牌測試二', 'propaganda', {'propaganda': 1}),
        Card('紅軍保留手牌', 'money', {'money': 1}),
    ]
    red.organizations = {'北京': 1}
    ...
    discard_result = game.resolve_pending_choice(red.id, [0, 1])
    first_build_choice = dict(game.pending_choice or {})
    first_build_index = next(i for i, entry in enumerate(first_build_choice.get('towns') or []) if entry.get('town') == '吉爾吉特')
    first_build_result = game.resolve_pending_choice(red.id, first_build_index)
    second_build_choice = dict(game.pending_choice or {})
    second_build_index = next(i for i, entry in enumerate(second_build_choice.get('towns') or []) if entry.get('town') == '阿里')
    second_build_result = game.resolve_pending_choice(red.id, second_build_index)
    checks.append(check(
        'tibet_red_discards_any_number_then_builds_same_count_near_tibet_org',
        runtime_effects.get('red_suppression', {}).get('status') == 'pending_discard_choice'
        and runtime_effects.get('red_suppression', {}).get('max_discard') >= 2
        and first_choice.get('choice_key') == 'era_red_discard_to_build_near_target'
        and first_choice.get('type') == 'multi_card_choice'
        and first_choice.get('count') >= 2
        and first_choice.get('min_count') == 1
        and discard_result.get('pending_choice') is True
        and discard_result.get('discarded_count') == 2
        and first_build_choice.get('choice_key') == 'era_red_build_near_target'
        and first_build_result.get('pending_choice') is True
        and first_build_result.get('remaining_builds') == 1
        and second_build_choice.get('choice_key') == 'era_red_build_near_target'
        and second_build_result.get('success') is True
        and second_build_result.get('remaining_builds') == 0
        and discard_count(red, '紅軍棄牌測試一') == 1
        and discard_count(red, '紅軍棄牌測試二') == 1
        and red.organizations.get('吉爾吉特') == 1
        and red.organizations.get('阿里') == 1
        and len(red.hand) == 1,
        {
            ...
            'runtime_effects': runtime_effects,
            'first_choice': {
                'choice_key': first_choice.get('choice_key'),
                'type': first_choice.get('type'),
                'count': first_choice.get('count'),
                'min_count': first_choice.get('min_count'),
                'prompt': first_choice.get('prompt'),
            },
            'discard_result': discard_result,
            'first_build_choice_towns': [entry.get('town') for entry in first_build_choice.get('towns') or []],
            'first_build_result': first_build_result,
            'second_build_choice_towns': [entry.get('town') for entry in second_build_choice.get('towns') or []],
            'second_build_result': second_build_result,
            'red_discard_counts': {
                '紅軍棄牌測試一': discard_count(red, '紅軍棄牌測試一'),
                '紅軍棄牌測試二': discard_count(red, '紅軍棄牌測試二'),
            },
            'red_orgs': dict(red.organizations),
            'red_hand_after': [getattr(card, 'name', str(card)) for card in red.hand],
            'era_effects_applied': game.turn_log.get('era_effects_applied'),
        }
    ))
