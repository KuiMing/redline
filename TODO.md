...
### P1：地圖縮放時固定「開新遊戲」位置
- [todo] 在戰略地圖執行 zoom in 或 zoom out 時，「開新遊戲」會偏離「我的陣營」與同列按鈕。按鈕必須維持同列、同高及靠視窗右側的位置。
- [todo] 定位時不可受 Leaflet 地圖縮放 transform 或地圖容器重繪影響；仍須保持在故障遮罩之上並可點擊。
- [todo] 補上 Browser regression，實際執行多次 zoom in／zoom out，逐次驗證按鈕位置、點擊命中及新遊戲導向。
