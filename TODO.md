...
- [todo] 後續需決定：若文案維持「殘部」語意，數字應只統計非紅軍陣營；若要顯示全體組織總數，文案必須明確標示為牆內／牆外總數。需同步驗證勝利摘要表、各觀看陣營視角及零組織邊界。

### P2：時代關卡提示移至事件卡左側
- [todo] 回報情境：事件卡顯示時，時代關卡提示浮在事件卡正前方，會遮住事件卡內容與操作區域。
- [todo] 期望：時代關卡提示改放在事件卡左側，不要遮擋事件卡；兩者需維持清楚的視覺關聯，並在不同 viewport 下保持完整可讀。
- [todo] 需檢查：事件卡與時代關卡提示的容器布局、定位基準、z-index、窄螢幕 fallback，以及提示出現／消失時是否造成事件卡跳位或裁切。
- [todo] 回報截圖：`docs/records/playtest-flow/era-gate-overlaps-event-card_20260820.jpg`
