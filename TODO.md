...
### todo
- [in_progress] 批判：補完整 UI 驗證與收尾整理。
- [todo] 批鬥：補完整 UI 驗證與收尾整理。
- [todo] 情報網：補完整 UI 驗證與收尾整理。
- [todo] 企業人脈：補完整 UI 驗證與收尾整理。
- [todo] 企畫遊說：補完整 UI 驗證與收尾整理。
- [todo] 模仿戰術：補完整 UI 驗證與收尾整理。
- [todo] 誘導虛耗：補完整 UI 驗證與收尾整理。
- [todo] 北國奧援：補玩家指定瓦解目標 UI／驗證與收尾整理。
- [todo] 臺灣奧援：補玩家選 town 的完整 UI／驗證與收尾整理。
- [todo] 天方奧援：補指定對手 / 範圍確認 UI／驗證與收尾整理。
- [todo] 紅軍奧援：把 text fallback 補成明確 engine effect，或明確標註規則待定。

## 狀態盤點／提醒（不要直接當成 todo）

### note
- [note] 北國奧援：部分實作。
  - 依據：`dissolve_many_near` / `dissolve_self_and_enemy` 已做。
  - 問題：目前會自動找敵方 town；缺玩家指定瓦解目標與合法性 UI 呈現。
- [note] 臺灣奧援：部分實作。
  - 依據：`dissolve_and_build` / `dissolve_many_near` 已做。
  - 問題：先瓦解再建設的目標點由 server 自動處理；沒有玩家選 town 的完整 UI。
- [note] 天方奧援：部分實作。
...
- [note] 部分實作（MVP / 自動化）。
  - Action：情報網、企業人脈、企畫遊說、模仿戰術、批判、批鬥、誘導虛耗。
  - Support：北國奧援、臺灣奧援、天方奧援。
