...
- [done] 修正香港在其他玩家回合按下遷移按鈕時，被 WebSocket 回合守門誤擋為 `Not your turn` 的問題。香港可遷入已有香港組織的目的地；該組織成為根據地，原根據地組織保留。戰略地圖以 `🏕` 標示所有參戰陣營目前的根據地，並在香港遷移後即時移動標示。正式 Browser proof **20/20 passed**、香港規則 validator **5/5 passed**、相關 pytest **10/10 passed**、完整 pytest **362/362 passed**，console 0 errors。原始回報與修正後截圖位於 `docs/records/faction-ui/hong-kong-base-relocation/`。
