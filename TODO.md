...
- [todo] 現行文字「香港抗爭之烈」錯誤；正確事件卡名稱是「香港抗暴之戰」。
- [todo] 規則語意：事件卡「香港抗暴之戰」發生並完成結算後，香港陣營可免費前移根據地。不可把這項效果寫成不存在的「香港抗爭之烈」規則。
- [todo] 實作前核對事件卡原文、觸發時點、可遷移地點、是否可維留香港城及效果持續時間，再同步修正 UI、規則說明與測試。
- [todo] 香港陣營的「我的陣營」初始說明必須列出根據地遷移機制，並分別說明各個可選根據地的特殊能力。玩家不應等到事件結算或遷移選擇出現時，才看到這些差異。
- [todo] 說明必須明確區分香港城目前能力、各個前移根據地能力、遷移解鎖條件，以及遷移後取代或保留哪些效果。
