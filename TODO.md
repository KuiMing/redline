...
- [done] 情報網：補完整 UI 驗證與收尾整理。
  - 2026-05-18：已修正第一選項改為至多 3 位其他玩家各獲得 1 張內鬥，補 `discard_pile` state 序列化與戰況總覽棄牌堆顯示；正式 UI 截圖已可同畫面證明 enemyA/enemyB/enemyC 各自「棄牌 1／棄牌堆 內鬥」。
...
- [note] 天方奧援：已完成指定對手 / 範圍確認 UI 與正式前中後對照證據。
  - 2026-05-17：已補齊操作前、選目標 modal、完成後三張正式 UI 截圖；III級可直接證明目標 `red` 從 2 張手牌降為 0 張，並在 action log 顯示隨機棄 2 張結果。
