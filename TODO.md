...
- [todo] 回報情境：玩家再次使用可建立組織的卡牌時，戰略地圖沒有停留在上一次操作完成後的中心與 zoom。玩家已在洛杉磯建立組織後，洛杉磯也成為合法建立範圍的一部分；每次進入建立組織流程時，系統似乎為了容納所有合法目標而直接 zoom out 到接近全地圖。
- [todo] 期望：連續使用建立組織卡時，保留玩家上一次的地圖中心與 zoom；系統仍可標示洛杉磯等遠端合法目標，但不應因合法目標分布很廣而自動重算為全地圖視角。
- [todo] 需檢查：建立組織 pending choice 啟動時的自動 `fitBounds`、合法目標集合更新、上一次地圖視角的保存／還原，以及卡牌選擇後切換戰略地圖時是否重複觸發全目標聚焦。
