...
- [todo] 玩家可能連續打出多張「可以建立組織」的卡牌；系統應先累積各張卡提供的待建立次數，再讓玩家於戰略地圖連續完成全部組織建立，而不是第一張卡產生 pending choice 後就阻止後續建立卡出牌。此能力需通用支援所有建立組織卡，不只特判 `宣傳家`。每建立一次後都要重新計算合法城鎮與組織棋供應，地圖顯示剩餘建立數，並保留各張卡其他效果及其正確結算順序。（2026-07-31 操作流程回報）

### P1：所有奧援卡的事件任務進度稽核
- [todo] 已觀察到目前事件為 `東突厥集中營`時，玩家打出 `臺灣奧援`沒有計入原本應滿足的任務條件；懷疑所有奧援卡都可能因共用的 support 類型判定而被排除於事件任務進度之外。後續不能只修 `臺灣奧援`特例：需逐一檢查所有奧援卡 × 所有適用事件任務條件，對照事件原文確認實際條件欄位，檢查出牌進度 hook、奧援卡費用／類型判定與成功結算時點，並補共用 runtime matrix 及代表性正式 UI regression。（2026-07-31 事件回報；後續補充為所有奧援卡可能同型）
...
- [done] 已將所有 8 種一般奧援卡、每種 2 個印刷 variant 的 II／III 級判定由「指定地區至少有 1 個己方組織」改為「指定地區組織數並列全場最多且大於 0」；平手視為共同最多，全員 0 不算。III 級先比較該卡自身奧援地區，II 級仍只檢查該實體卡印刷的兩個指定地區並維持 OR 語義；`matched_rulers`只回傳真正領先的地區。區域計數統一經實體組織城鎮與共用關係計算，依 `rules.md`讓共用組織對每個共享玩家各計 1，但同一玩家不重複計算同一實體。新增 8 卡 × 2 variants matrix，涵蓋有 1 但落後、並列最多、自身地區 III 級、零組織、OR 及共用組織；focused pytest 4/4、affected pytest 119/119、奧援 runtime 12/12、東洋互動 6/6、南洋 2/2、北國 7/7、一城一組織 11/11、正式 UI proof 9/9。UI 證據位於 `docs/records/support-region-leadership/`。（2026-08-01 完成）
