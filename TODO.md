...
- [done] 回報紀錄：Turn 14 觸發 `[臺灣]綏靖派反對介入對岸`，並將 3 張「內鬥」加入臺灣玩家棄牌堆；實際觀察中，革命反撲效果只維持一回合，Turn 15 後已不再持續。
- [done] 規則依據：`data/cards/event_and_era_cards.v1.1.json` 與 `data/raw/event_and_era_cards.csv` 均明載「每當臺灣在臺灣城鎮建立至少1個組織時，獲得1點宣傳。持續2回合。」；`data/era_structured.v1.1.json` 同時設定關卡 `duration.value = 2` 及效果 `duration = 2`。
- [done] 根因與修正：舊邏輯在每位玩家結束個別回合時扣減時代期限。現在只在紅軍回合結束的完整輪次邊界扣減，而且先扣既有關卡，再偵測新關卡，避免新觸發的 2 回合效果立即降為 1 回合。
- [done] 驗收覆蓋：觸發當下只執行一次紅軍壓制；臺灣第 1、2 個有效回合在臺灣城鎮建立組織均可獲得 1 點宣傳；第 2 個完整輪次結束後才失效；active-era 投影、剩餘回合與遊戲紀錄一致。
- [done] 交付：同一份行為修正與完整時間線回歸已合併到 `main` 與 `dev`。核心修正 commit 為 `f92579e`；後續時間線測試由 `16ea64f`、`12cf8eb`、`4e09e2e` 補強。
- [done] 2026-09-01 現況驗證：最新 `origin/main` (`9878bcc`) 與最新 `origin/dev` (`ae6a8a4`) 執行 `uv run pytest -q scripts/tests/test_era_lifecycle.py`，兩者皆為 **14 passed**。
...
### P1：紅軍能力成功後不顯示結果列
- [done] 統戰部、政工部、國安部與中紀委成功結算後，不再建立 HUD 結果提示或陣營面板結果文字。
- [done] 等待對手決定是否取消紅軍能力時，改用可由右上角叉叉關閉的視窗，不再占用 HUD 結果列。
- [done] 對手以爆料黑幕等反應卡取消紅軍能力後，紅軍會看到取消玩家、反應卡與能力名稱；視窗可由右上角叉叉關閉。
- [done] 關閉等待視窗後，同一個反應等待不會因 WebSocket 狀態同步而重新彈出；反應結束後可顯示新的取消結果視窗。
- [done] 紅軍能力計數、手牌、牌庫與組織等實際狀態仍正常更新；無法發動時仍保留必要的阻擋提示視窗。
- [done] 精簡紅軍介面與反應視窗 Browser proof **12/12 passed**；紅軍能力無可執行內容回歸 **6/6 passed**；紅軍特殊規則 runtime **6/6 passed**。

### P1：根據地改為直接自由選擇
- [done] 移除「返回根據地類別」與泛稱根據地類別按鈕；「任意牆內」依正式地圖統治者展開為真實牆內城鎮，選擇陣營後直接列出所有合法城鎮。
- [done] 玩家可直接在不同城鎮之間反覆切換；目前選取狀態、Lobby 摘要與右側根據地說明會立即同步更新。
- [done] 已被其他玩家選擇的根據地會 disabled；後端在確認時再次拒絕競爭條件，不再等到開局才報錯。
- [done] 修正單一根據地按鈕因 flex 換行列預設拉伸而填滿整個高度；北京等單一按鈕維持正常尺寸。
- [done] 所有陣營的根據地外框統一為反賊配置的 116px 高度；內容超過時只在框內捲動。
- [done] 陣營變體與根據地各自使用內部捲動區，1280×720 與 1024×768 均保留可操作的根據地按鈕區。
- [done] 專用 Browser proof **18/18 passed**；1,371 種陣營／實際根據地組合 **1,371/1,371 passed**；房主難易度 **8/8 passed**；最後席位紅軍限制 **7/7 passed**；完整 pytest **418/418 passed**。

### P1：「展現實力」由能力擁有者選擇獎勵
- [done] 文案統一為「展現實力：在己方行動階段打出至少3張不同名稱的非起始牌，獲得3點宣傳或3點資金」。
- [done] 達成條件後不再自動獲得資金；由能力擁有者選擇「獲得 3 點宣傳」或「獲得 3 點資金」，後端驗證選擇者與選項。
- [done] 選擇會等待既有 `pending_choice` 完成，不覆蓋卡牌、事件、時代或其他陣營能力選擇；每回合仍限觸發一次。
- [done] 兩條 Browser choice 路徑 **2/2 passed**；真實舊 choice 結算後自動推出展現實力的排隊路徑已覆蓋；專用單元與資料投影測試 **39/39 passed**；完整 pytest **418/418 passed**。

### P1：陣營選擇視窗顯示時代關卡
- [done] 陣營資料 API 由正式時代關卡與卡牌資料投影對應陣營的名稱、觸發條件、紅軍壓制及革命反撲；不額外套用可能誤述個別分支的通用期限。
- [done] 大型陣營／根據地選擇視窗加入時代關卡區塊；紅軍清楚標示沒有個人時代關卡。
- [done] 整合 Browser proof **18/18 passed**；1,371 種陣營／實際根據地組合 **1,371/1,371 passed**；1280×720 與 1024×768 無裁切。

### P1：以大型視窗選擇陣營與根據地
- [done] 建立房間與加入房間後，自動開啟大型遊戲內選擇視窗；恢復尚未完成選擇的席位時也會自動開啟。
- [done] 左欄分步選擇陣營與根據地；右欄同步顯示完整能力、規則限制與獲勝條件，不再受原本 Lobby 小區塊高度限制。
- [done] 確認後關閉視窗並返回 Lobby；Lobby 提供「選擇／變更陣營與根據地」按鈕，重新開啟時恢復目前選擇。
- [done] 保留伺服器的陣營占用、根據地、最後席位紅軍限制與準備條件驗證；Escape 與「返回作戰室」只關閉視窗，不送出未確認選擇。
- [done] 專用 Browser proof **10/10 passed**；454 種陣營／根據地組合 **454/454 passed**；房主難易度 **8/8 passed**；最後席位紅軍限制 **7/7 passed**。
- [done] 1280×720 與 1024×768 均無裁切；證據：`docs/records/faction-ui/faction-picker-modal/`。

### P1：待選效果改用提示視窗
- [done] 玩家因尚有待選效果而執行其他操作時，不再只顯示 HUD 金色文字或瀏覽器原生 alert。
- [done] 待選效果屬於本人時，重新開啟可直接操作的既有選擇視窗；地圖選擇則提示前往戰略地圖完成。
- [done] 待選效果屬於其他玩家時，顯示「等待其他玩家處理效果」視窗，並顯示負責玩家名稱。
- [done] 提示視窗可關閉、焦點移至「知道了」、不建立或消耗任何選擇。
- [done] Browser proof **6/6 passed**；既有無合法能力視窗回歸 **6/6 passed**；完整 pytest **413/413 passed**。

### P1：游擊隊由能力擁有者選擇效果
- [done] 玩家建立至少 1 個牆內組織並首次觸發「游擊隊」時，不再自動令紅軍棄牌。
- [done] 顯示兩個選項：「抽 1 張牌」與「令紅軍棄 1 張手牌」；選擇權屬於觸發能力的維吾爾／藏國玩家。
- [done] 紅軍沒有手牌時，自動執行仍可完成的「抽 1 張牌」；每回合限 1 次規則保持。
- [done] 建立組織仍有待決流程時，游擊隊選擇會排隊等待，不會覆蓋卡牌、奧援或連續建立選擇。
- [done] 游擊隊與陣營能力觸發路徑 **34/34 passed**；既有陣營能力驗證 **4/4 passed**；完整 pytest **413/413 passed**。

### P1：時代關卡改於紅軍回合結束後判定
- [done] 非紅軍玩家達成組織門檻後，不在其回合結束或事件階段立即排入時代關卡；保留紅軍同一回合完整的瓦解機會。
- [done] 紅軍回合結束後才重新依目前盤面判定。若紅軍已將藏國牆內組織降至 7 個以下，`[藏國]藏國騷亂` 不觸發。
- [done] 紅軍為整輪最後一席時，時代關卡與效果使用下一回合編號；第 13 回合紅軍結束後，紀錄顯示 `[Turn 14] Era triggered: [藏國]藏國騷亂`。
- [done] 互動式時代關卡先由紅軍完成，再交棒給下一位玩家；既有多關卡序列與延後事件不回退。
- [done] 專用紅軍判定時機與藏國失效回歸 **2/2 passed**；時代關卡相關測試 **39/39 passed**；完整 pytest **411/411 passed**。

### P2：統一「非暴力」能力文案為武裝類卡牌
- [done] 將西藏達蘭薩拉與維吾爾慕尼黑的「非暴力」能力由「禁止持有或裝備類卡牌。」改為「禁止持有武裝類卡牌。」；同步更新整合陣營資料。
- [done] 打出與購買限制的玩家錯誤訊息同步改為「不能打出／購買武裝類卡牌」，避免能力說明與操作提示用詞不一致。
- [done] 四個非暴力陣營文案與操作錯誤專用測試 **2/2 passed**；既有非暴力能力驗證 **4/4、7/7 passed**；完整 pytest **409/409 passed**。

### P1：紅軍根據地組織被瓦解後移除地圖根據地標示
- [done] 紅軍根據地第二次成功瓦解並移除實體組織後，戰略地圖不再顯示該城鎮的帳篷標示或根據地陣營外框；後端仍保留根據地位置資料，供下一回合重建規則使用。
- [done] 紅軍日後在原根據地重建組織時，標示會依有效根據地組織自動恢復；其他仍有組織的參戰陣營根據地不受影響。
- [done] 蒙古同一回合第二次瓦解北京後，張家口 → 北京鐵路遷移列為合法且花費 1 次移動；專用單元測試覆蓋實際遷移成功。
- [done] Browser proof 覆蓋瓦解前／瓦解後、蒙古實際從張家口遷入北京、1280×720、1024×768、其他根據地保留、北京合法遷移與 console 0 errors，**10/10 passed**。證據：`docs/records/map-ui/destroyed-red-base-marker/`。

### P2：將「卡牌置頂」移到「清除」旁邊
- [done] 將原本獨立顯示在 HUD 下方的「頂牌」按鈕移入隨機購買區控制列，放在「清除」左側。
- [done] 按鈕文字改為「卡牌置頂 (N)」，保留剩餘使用次數、候選牌判斷、disabled 狀態與既有選牌流程。
- [done] 只有卡牌置頂權利時不再開啟獨立 `phaseActionBar`，因此不會壓低指揮中心內容；其他真正需要 HUD notice 的訊息仍可使用該列。
- [done] 專用 Browser proof：1280×720、1024×768、按鈕相鄰關係、無獨立列與既有選牌流程 **6/6 passed**，console 0 errors。
- [done] 回歸：頂牌選擇規則 **7/7 passed**；回合結束頂牌流程 **5/5 passed**；購買 affordance Browser proof **3/3 passed**；完整 pytest **406/406 passed**。證據：`docs/records/ui-layout/topdeck-near-clear/`。

### P2：勝利摘要文字加大並改善資訊密度
- [done] 回報情境：遊戲結束畫面的勝利摘要文字偏小；摘要面板下方留下較多空白，訊息空間看起來太空。
- [done] 放大「遊戲結束」、勝利者標題、回合資訊、結局標題、結局敘述、玩家統計與「檢視最終盤面」按鈕；增加摘要列高度與內距。
- [done] 玩家統計在右欄垂直置中，將原本集中在上方的內容重新分配到面板高度，減少單側大片空白。
- [done] 5 位以上玩家自動套用多人緊湊尺寸；長敘述自動降低內距與字級。一般結局面板最多使用畫面 54%，紅軍災難視角仍維持 48% 上緣面板，避免遮住下半部難民主體。
- [done] 保留全畫面勝利背景、觀看最終盤面按鈕、玩家統計內容與 viewer-specific 勝利敘述；長敘述與多人統計維持可捲動，不會以 hidden 裁切。
- [done] 專用 Browser proof 覆蓋兩位玩家、長敘述、六位玩家、1280×720 與 1024×768，共 **15/15 passed**，console 0 errors。
- [done] 回歸：勝利敘述 viewer scope **13/13 passed**、紅軍雙視角組織統計 **7/7 passed**、完整 pytest **406/406 passed**。
- [done] 證據：`docs/records/ui-layout/victory-summary-text-density/`；保留原始需求圖、兩種解析度截圖及結構化驗證報告。
...
- [done] 無可執行內容時回傳統一的 `unavailable` 結果與具體繁體中文訊息。能力不扣次數、不記錄事件進度、不消耗卡牌或供應、不開啟取消反應、不建立空選擇，也不留下目標選擇 modal 或地圖 targeting 狀態。
- [done] 2026-08-23 UI 調整：`unavailable` 結果改用置中、阻擋式的「行動提示」視窗，標題顯示「能力名稱＋無法發動」，並提供「知道了」、背景點擊及 Esc 關閉。提示開啟後焦點移到關閉按鈕，關閉後恢復原焦點。
- [done] 無合法目標不再寫入 `#phaseActionNotice`，因此不會在 HUD 控制列突然增加黃色提示內容；也不再於陣營能力資訊區重複顯示。其他成功的陣營能力結果仍沿用 5 秒 sticky notice。
...
- [done] 驗證：單元測試 **5/5 passed**；紅軍能力正常與無供應流程 **8/8 passed**；無內容 modal Browser proof **6/6 passed**；全畫面狀態列 **10/10 passed**；完整 pytest **406/406 passed**；Browser console 0 errors。證據：`docs/records/playtest-flow/red-army-empty-ability-feedback/`。
...
- [done] 根因：`VictoryEngine.evaluate()` 在檢查各陣營明確勝利條件以前，就先判定 `turn > 20` 的紅軍生存勝利。紅軍生存條件因此成為無條件優先勝利，而不是在無其他陣營達標時才使用的保底勝利。
- [done] 修正：整輪結束時先檢查所有玩家的明確陣營勝利條件；只有無人達標時，才判定紅軍第 20 回合生存勝利。原本「必須等紅軍完成本輪行動」的 round-wrap 時機不變。
...
- [done] 互動：Tab 顯示時代關卡名稱與剩餘回合。點擊任一 Tab 會開啟對應的完整時代關卡說明；「縮到右上角」改為「縮到分頁列」。時代關卡結束後，其 Tab 會隨狀態更新移除。
- [done] 布局：3 個時代關卡 Tab 可同時放入 1280 設計分頁列，並沿用 stage 縮放支援 1024×768。Tab 與右上角事件卡位於不同列，不再使用 overlay，因此不會互相遮擋。
- [done] 驗證：正式 session 恢復 Browser proof **4/4 passed**，涵蓋 3 個時代關卡同時生效、各 Tab 開啟正確說明、1280×720、1024×768 及 console 0 errors；我的時代關卡 proof **9/9 passed**；事件卡 proof **13/13 passed**；完整 pytest **370/370 passed**。修正後截圖：`docs/records/playtest-flow/three_era_tabs_clear_of_event_card_1280x720_20260820.png`、`docs/records/playtest-flow/three_era_tabs_clear_of_event_card_1024x768_20260820.png`。原始回報：`docs/records/playtest-flow/era-gate-overlaps-event-card_20260820.jpg`。

### P2：連續使用建立組織卡時保留上一次地圖視角
- [done] 根因：`applySupportChoiceHighlight()` 只記住目前 pending choice 的 focus key。建立 session 結束時 payload 會清空並將 key 重設為 `null`；下一張建立組織卡產生新的 pending choice 後，即使玩家已手動選好洛杉磯附近的中心與 zoom，系統仍把它當作需要第一次聚焦的新互動，再次對香港與洛杉磯等全部合法目標執行 `fitBounds`，因此縮到接近全圖。
- [done] 修正：每個已載入的遊戲／玩家地圖只讓第一個建立組織互動自動聚焦一次。之後不同卡牌及不同 pending build session 只更新全部合法候選 marker、提示及剩餘建立數，不再修改玩家目前的 Leaflet `center`／`zoom`。切換至另一場遊戲或另一位玩家時會重設此狀態，保留新遊戲首次建立的自動定位。
...
- [done] 根因：紅軍進入「一帶一路 南洋」建立流程時，地圖沿用通用 `fitBounds` 與 `[110, 110]` padding，造成初次視角過廣。第一次修正誤用固定南洋核心 `[8, 104]`、`zoom 5`，只顯示 11 個合法城鎮中的 6 個，沒有符合「聚焦紅軍當下可建立組織的城鎮」的規則。
- [done] 修正：建立互動 payload 保留 canonical `region`。當互動符合 `event_build_organization`、`region: southeast_asia` 與來源「一帶一路 南洋」時，地圖直接使用目前 pending choice 中紅軍實際可建立組織的城鎮計算 bounds、中心與 zoom。聚焦不使用固定城鎮集合；已被占用而不能建立的城鎮不參與計算。窄螢幕可自動降低 zoom，候選較少時最多放大至 `zoom 6`。此事件具有專用聚焦優先權；同一事件內不會反覆搶走視角，非行動玩家仍保留自己的根據地視角。
- [done] 驗證：正式 create／join／start／session restore proof 先完成既有建立 session，將地圖手動設為北京 `zoom 9`，再發動事件。現有 11 個合法城鎮的實際 bounds 為北緯 22.4856 至南緯 6.2088、東經 96.1561 至 120.9842；地圖正確聚焦其中心 `[8.1384, 108.57015]`、`zoom 4`，11/11 合法候選全部在 viewport 內。集中聚焦 Browser proof **7/7 passed**；既有南洋地圖 proof **7/7 passed**；紅軍回合事件建立／過期候選復原 Browser regression 成功；跨建立 session 視角 proof **6/6 passed**；完整 pytest **379/379 passed**；Browser console 0 errors。報告：`docs/records/event-cards/belt-road-nanyang-focus/BELT_ROAD_NANYANG_CONCENTRATED_FOCUS_VALIDATION.json`；截圖：`docs/records/event-cards/belt-road-nanyang-focus/belt_road_nanyang_concentrated_focus_20260822.png`。
...
- [done] 使用者回報：戰略地圖選「陣營」篩選為香港後按「聚焦結果」，畫面不會真的 zoom in
  ...
  東沙、澳門、倫敦），但聚焦計算應該更看重香港本身。
- [done] 根因：`fitVisible()` 聚焦中心早就用「結果座標中位數」計算，但縮放層級仍沿用
  「全部篩選結果」原始 bounds 算出的 `fittedZoom`，只是事後夾在 4~9。新增
  `coreClusterTowns()`：以中位數為中心、用「與中心距離的中位數」代表主要群集典型半徑
  （對極端離群點如倫敦穩健），超過典型半徑 4 倍的城鎮排除在聚焦計算外，但完整篩選結果
  仍不變地留在地圖上，玩家可平移查看。第一版用「排序找最大間隔」分群，被倫敦這個極端
  離群點主導間隔位置而失敗（`coreCount: 26/27`，zoom 仍只有 5），改成距離門檻後才正確
  （`coreCount: 18/27`，zoom 提升到上限 9）。反賊／紅軍這類本來就遍布全圖的陣營門檻內
  幾乎涵蓋全部結果，維持原本縮放層級，沒有回歸問題。完整 pytest 366/366 passed。詳見
  `docs/records/map-ui/camp-filter-focus/CAMP_FILTER_CORE_CLUSTER_FOCUS_VALIDATION.md`。

### P1：沒有合法城鎮時停用建立組織行動卡
- [done] 宣傳家、思想家、組織經驗丙、組織經驗乙及組織經驗甲在完全沒有合法建立城鎮時，不能作為行動使用。後端在卡牌離開手牌前拒絕操作，並保留既有待決建立佇列。
- [done] 手牌的行動按鈕改為停用的「無城鎮可建立」，提示「目前沒有城鎮可以建立組織。」；資源模式仍可使用。
- [done] 合法城鎮由後端依卡牌距離、城鎮占用、陣營發展空間、組織棋供應、事件及其他限制即時計算。正式 Browser proof **6/6 passed**；行動卡回歸 **149/149 passed**；建立佇列與行動卡聚焦回歸 **161/161 passed**；完整 pytest **366/366 passed**（連續重跑 20 次穩定通過，無不穩定情形）。
- [done] 事件卡觸發建立組織（`build_organization_in_region`／`build_organization_near_own`）、卡牌連續建立額度用盡（排隊中／組織經驗甲重複建立）等沒有合法城鎮時，原本只留一行英文 log，改成繁體中文，跟其餘介面一致。
- [done] 修掉既有測試 `test_industry_infiltration_is_offered_as_a_candidate_for_propaganda_only_static_card` 的不穩定性：`make_game()` 沒有明確指定紅軍玩家的起始據點時，紅軍起始組織有時是 `{'北京': 1}`、有時是空的，這個隨機性以前無關緊要，但新加的建立組織合法性檢查會因此偶爾在測到反應視窗之前就先擋下打牌——明確指定起始據點為北京（紅軍唯一的起始根據地）後穩定。

### P1：宣傳家建立範圍自動聚焦
- [done] 宣傳家取得建立權後，即使手上仍有其他可排隊的建立牌，也會直接切換至戰略地圖並聚焦後端判定的合法建立城鎮。玩家仍可手動回到指揮中心繼續排入其他建立牌。
- [done] Browser proof 使用「宣傳家＋另一張可排隊建立牌」及「choice 早於地圖資料完成」的新遊戲首次載入情境，確認直接切換地圖、資料就緒後重試聚焦、延遲的亞洲預設視角不會覆蓋、保留排隊能力及 console 0 errors，結果 **6/6 passed**；原多張建立牌 Browser regression **10/10 passed**。

### P1：軍火庫城鎮的地圖圖示
- [done] 地圖資料中全部 16 座 `type == "軍火庫"` 城鎮會常態疊加 🧨。圖示使用獨立 Leaflet layer，綁定同一城鎮座標並置於城鎮標記右上方。
- [done] 軍火庫成為瓦解目標時，💀 保持置中且位於較高層級；🧨 同時保留，不會取代或遮掉瓦解狀態。
- [done] Browser regression 驗證 16/16 軍火庫、3 次 zoom in、2 次 zoom out、🧨＋💀 同時顯示及 console 0 errors，結果 **7/7 passed**。所有縮放階段的圖示座標一致，畫面偏移固定為 `(+15px, -13px)`。

### P1：修正香港根據地遷移用語
- [done] 將香港根據地事件與 UI 中的「前移」統一改為「遷移」。包含「免費前移根據地」→「免費遷移根據地」、「可前移」→「可遷移」，以及「前移至臺北／倫敦／卡加利／多倫多」→「遷移至臺北／倫敦／卡加利／多倫多」。
- [done] 將「維留」統一改為「留在」。包含提示文字「亦可維留目前的香港城」→「亦可留在目前的香港城」，以及按鈕「維留香港城」→「留在香港城」。
- [done] 依截圖逐項檢查分頁名稱、事件標題、提示文字及五個操作按鈕。原始截圖證據：`docs/records/faction-ui/hong-kong-relocation-wording/playtest-wording-20260816.jpg`；修正後證據：`docs/records/faction-ui/hong-kong-base-relocation/hong_kong_base_relocation_ui.png`。
- [done] 已同步更新 UI、伺服器訊息、陣營資料、正式規則、驗證器與回歸測試。

### P1：修正「香港抗暴之戰」事件名稱與香港根據地效果
- [done] 正式名稱已統一為事件卡「香港抗暴之戰」；陣營資料、`rules.md`、UI 與 runtime 不再把「香港抗爭之烈」當作有效事件名稱。
- [done] 「香港抗暴之戰」完成結算後，不論任務成功或失敗，香港可在下一回合開始前免費遷移一次根據地至臺北、倫敦、卡加利或多倫多；亦可明確選擇留在目前根據地。空目的地可遷入；目的地已有香港自己的組織時，該組織直接成為新根據地，原根據地組織保留；目的地有其他陣營組織時仍不可遷入。
- [done] 已分離兩種規則：事件負責免費遷移根據地；赤鱲角機場只負責花費 2 次遷移移動香港組織，不能再被誤用為付費遷移根據地。
- [done] 大廳陣營選擇與遊戲內「我的陣營」均會列出香港城、臺北、倫敦、卡加利及多倫多的能力、目前根據地、可遷移根據地、解鎖事件及遷移後能力切換。
- [done] 事件窗口開啟時，「我的陣營（可遷移）」會亮起，並提供四個遷移按鈕及「留在」按鈕。Browser proof **9/9 passed**；單元與 board-action 聚焦回歸 **8/8 passed**；規則 validator **5/5 passed**。
...
- [done] 修正香港根據地遷移的換人時序。香港按下「結束回合」後，系統先完成補牌與「香港抗暴之戰」事件效果；若有必要棄牌，先完成棄牌，再要求香港選擇遷移或留在。香港完成決定後，才將行動權交給下一個陣營。香港可遷入已有香港組織的目的地；該組織成為根據地，原根據地組織保留。遷移按鈕維持簡潔的「遷移至臺北」，不額外標示「已有香港組織」。戰略地圖以 `🏕` 標示所有參戰陣營目前的根據地，並在香港遷移後即時移動標示。正式 Browser proof **23/23 passed**、香港規則 validator **6/6 passed**、香港聚焦 pytest **10/10 passed**、完整 pytest **364/364 passed**，console 0 errors。原始回報與修正後截圖位於 `docs/records/faction-ui/hong-kong-base-relocation/`。
...
### P1：戰略地圖陣營篩選聚焦
- [done] 修正「聚焦結果」對陣營篩選看似沒有反應。陣營發展範圍含海外離群據點時，改以篩選結果的座標中位數聚焦主要群集，並保證至少 zoom 4；全部篩選結果仍保留，可平移查看海外據點。九個陣營正式 Browser proof **10/10 passed**，console 0 errors。

### P1：地圖縮放時固定「開新遊戲」位置
- [done] 「開新遊戲」保留在縮放舞台外的最高層，但遊戲顯示期間每 100ms 重新讀取「我的陣營」與分頁列的實際矩形。Leaflet zoom、iframe 重繪或延後版面位移後，按鈕會自動恢復同列、同高及靠視窗右側的位置。
- [done] 定位不依賴 Leaflet transform；按鈕仍位於故障遮罩之上並可點擊。
- [done] Browser regression 實際執行 3 次 zoom in、2 次 zoom out 及延後 12px 分頁列位移；上下邊界均維持 0px 誤差。完整新遊戲 Browser proof **10/10 passed**。

### P1：大廳必須包含紅軍
- [done] 啟動行動的必要條件：所有玩家完成選擇及準備後，房間仍必須有且只能有一名紅軍。前端停用房主的「啟動行動」並顯示原因；伺服器 `/start` 同時強制拒絕，不能繞過 UI。
- [done] 若第四名玩家加入滿 4 人房間時尚無紅軍，該最後席位只能選擇紅軍。前端停用其他所有陣營並顯示原因；伺服器 `/choose-faction` 同時拒絕非紅軍選擇。若房內已有紅軍，第四名玩家仍可正常選擇其他未被占用的陣營。
- [done] 驗證：API 單元 **3/3 passed**；正式 Browser proof **7/7 passed**，涵蓋最後席位 UI 強制紅軍、伺服器拒絕繞過、紅軍可正常選擇、無紅軍時啟動按鈕停用、伺服器拒絕啟動及 console 0 errors。
...
- [done] 2026-08-13 playtest 修正：恢復席位功能上線後，戰略地圖 iframe 仍以無權杖 WebSocket 連線，遭伺服器拒絕後持續重連，並把英文授權錯誤顯示為通用「操作失敗」。地圖現已接收恢復權杖；父頁與地圖可同時保有各自 socket，廣播會送達兩者。權杖改用 WebSocket 子協定傳遞，不再出現在 URL access log。
...
### P0：時代關卡提示應該點分頁上方的提示列，不是右上角釘選卡片；並修掉連帶發現的既有 layout 重疊 bug
- [done] 使用者追問：「所以，我要點擊哪裡才看得到時代關卡的說明？原本指揮中心和戰略地圖的Tab 上方，好像會出現時代關卡相關的提示，感覺應該要按這個提示來放大時代關卡的說明比較好，而且萬一有四個人玩的話，有可能三個時代關卡同時觸發」——延續前一項（見下方）的「達成畫面可重新叫出說明」修法，指出更自然的入口應該是分頁上方本來就有的提示列，並提醒要驗證多個時代關卡同時生效的情境。（2026-08-09 使用者提出）
  - **確認並採納使用者建議**：`static/app.js` `render()` 裡 `.hud-era-row`／`.hud-era-pill`（指揮中心/戰略地圖分頁上方，`state.active_era_details` 逐一渲染的提示列）先前是純文字 `<span>`，完全無法點擊；改成跟 `#eraPinnedNotice` 釘選卡片共用同一套機制，`<button onclick="openEraAchievementById(item.id)">`，點哪個提示就開哪個時代的完整說明，兩種入口並存。
  - **過程中另外抓到一個更深層、比這次改動本身更早就存在的既有 layout bug**：`#topBar`／`#hud`／`#phaseActionBar`／`#gameShell`（含分頁列）是一整串寫死 `top` 像素值疊起來的絕對定位（48→90→150px），全部假設 HUD 剛好是單行高度。只要有任何一個時代關卡生效、`.hud-era-row` 一出現，HUD 就會多長出一整行，但下面每一層都還疊在原本假設的固定高度上——不只 `#phaseActionBar` 會整個蓋住時代關卡提示列本身（畫面上完全看不到、也點不到），連 `#gameShell`／分頁列都會反過來蓋住 `#phaseActionBar` 的按鈕（截圖看起來像有東西、`document.elementFromPoint()` 一測發現點到的其實是分頁列，不是按鈕）。這正是使用者「好像會出現提示」「所以我要點哪裡」會感到困惑的根本原因——不是這次新按鈕的問題，是這條提示列從被加進遊戲那天起，只要時代關卡一生效就一直處於「看得到內容、但視覺上被蓋住摸不清楚、也可能點不到」的狀態，只是先前沒人拿真實截圖＋`elementFromPoint` 仔細對過。
  - **修法**：`render()` 裡改成每次重繪都依 HUD／`#phaseActionBar` 實際量到的高度（`offsetTop`＋`offsetHeight`）逐層動態推算 `#phaseActionBar.style.top` 與 `#gameShell.style.top`，不再寫死像素值；不論時代關卡數量（0、1、或換行後的多行）都會正確逐層往下推，彼此不再重疊。`server/main.py` 的 `/test/setup-era-restrict-ignore-distance-proof` 新增選填 `extra_eras`（時代 id 陣列），可與 `era` 一併啟用多個時代，用於驗證四人局最多 3 個時代關卡同時生效的情境。
  - 驗證：新增 `scripts/validate_era_hud_row_clickable.py`：①三個時代關卡（反賊／哈薩克／滿洲）同時生效，指揮中心提示列正確顯示 3 個提示，用 `document.elementFromPoint()` 確認提示本身、不是別的元素蓋在上面，分別點擊哈薩克與滿洲的提示各自正確開出對應時代的完整說明；②單一時代關卡生效時，確認「結束目前步驟」按鈕仍然可點、沒有因為這次的動態定位改動被意外蓋住。**2/2 全過**（重跑 3 次穩定）。完整 pytest（同 baseline ignore 清單）**334/334 全過**（純前端修正，未動任何後端規則邏輯）；重跑既有 `scripts/validate_era_restrict_ignore_distance.py`（7/7）、`scripts/validate_safehouse_range_frontend.py`（4/4）、`scripts/validate_faction_action_modal_not_forced.py`（5/5）確認這次的版面調整沒有牽動其他既有畫面。

### P0：思想家／東洋奧援／組織經驗甲 沒有正確遵守反賊、哈薩克等時代關卡的「無法無視距離建立牆內組織」
- [done] 使用者 playtest 回報：「思想家、東洋奧援、組織經驗甲在觸發反賊時代關卡時，就應該無法讓玩家無視距離建立牆內組織，只能在原有組織附近一格或兩格內（如果有觸發增加距離的能力時）建立組織，牆外則是可以繼續無視距離建立組織。請再檢查一下，是不是有按照這個規則處理，也確保這些卡牌在類似的時代關卡觸發時，都能正常運作。」（2026-08-09 使用者 playtest 回報並要求修正）
  - **根因**：這條限制在遊戲裡有兩個來源，實作卻只把其中一個做對。①陣營能力「新疆社會管控」（維吾爾）走 `_faction_restricts_ignore_distance_build()`，`_card_build_town_choices()` 對它已正確地「不是整片排除，而是退回卡面的 `inner_fallback_range` 近距離」；②時代關卡的 `restrict_ignore_distance_build`（`data/era_structured.v1.1.json` 的 `rebels`＝[反賊]公知世代的終結、`kazakh`＝[哈薩克]伊塔事件，兩者結構完全相同）走 `_era_restricts_ignore_distance_build()`，但同一個迴圈裡對它只寫了一行 `continue`——**整片牆內直接排除、完全沒有退回近距離**，等於把「不能無視距離」誤解成「完全不能建立」。更嚴重的是 `_interactive_support_build_towns()`（東洋奧援 tier-3 `interactive_build_anywhere_inner`／tier-2 `interactive_build_near_inner` 的候選來源）**從頭到尾沒有呼叫過 `_era_restricts_ignore_distance_build()`**，只檢查陣營能力，所以反賊時代生效中的反賊玩家用東洋奧援 tier-3 仍可在整個牆內任意建立，完全繞過紅色壓制。另外 `data/action_cards_structured.v1.1.json` 的「思想家」缺 `inner_fallback_range` 欄位（姊妹牌「組織經驗甲」有 `1`），使得 `int(effect.get('inner_fallback_range', 0) or 0)` 取到 0、被視為「不允許退回→整片封死」——這也是當初時代關卡那條路徑會被寫成硬排除的原因。
  - **修法（不寫死 rebels／kazakh，任何未來同型效果的時代關卡都自動適用）**：`server/game.py` 新增兩個共用輔助 `_ignore_distance_build_restricted(player, town)`（陣營能力 or 時代關卡，任一成立即為受限）與 `_restricted_build_fallback_towns(player, fallback_range)`（退回範圍＝卡面格數＋`build_range_bonus`＋安全屋 +1），把原本只給陣營能力用的退回邏輯抽出來共用；`_card_build_town_choices()` 改成「受限→查退回範圍」而非「時代受限→硬排除」；`_interactive_support_build_towns()` 改為逐城鎮判定（不是整份清單一刀降級），未受限的城鎮仍維持無視距離，受限城鎮則以同一套近距離退回範圍過濾——逐城鎮寫法是為了讓 `scope` 不是整個牆內的未來時代效果也能正確運作。`data/action_cards_structured.v1.1.json` 的「思想家」補上 `"inner_fallback_range": 1`（該檔為 structured cards 唯一來源，已 grep 確認無重複副本）。`_era_restricts_ignore_distance_build()` 內既有的 `scope` 判定未動，牆外目標依舊回傳 False、維持無視距離。卡牌建立與奧援互動兩條路徑的**解析端**（`_live_pending_town_choices()`／`_resolve_support_interaction_result()`）本來就是重算同一組候選函式，因此修這兩個函式同時修好了「提供選項」與「實際結算驗證」。
  - **改寫既有錯誤測試**：`scripts/validate_xinjiang_distance_restriction.py` 的 `case_ideologue_excludes_inner_for_restricted()` 原本斷言 `'restricted_gets_no_inner_towns': not (towns & inner_towns)`（把「整片牆內都拿不到」當成正確答案），這正是上述誤解被寫死成測試的地方；已整支改寫為 `case_ideologue_inner_fallback()`，改斷言 `inner_offers_present_not_empty`＋`inner_offers_limited_to_1_step`＋`distant_inner_excluded`＋`outer_wall_still_ignore_distance`，比照隔壁本來就正確的 `case_org_experience_a_inner_fallback()`。
  - 驗證：`scripts/validate_xinjiang_distance_restriction.py` 從 4/4 擴充為 **10/10 全過**（新增時代關卡判定本身的 scope／陣營範圍、rebels 與 kazakh 兩個時代各自對思想家＋組織經驗甲的降級、兩個時代對東洋奧援 tier-3 的降級、`build_range_bonus` 讓退回距離變 2 格、以及反賊時代生效中非 rebel 陣營玩家完全不受影響）。完整 pytest（同 baseline ignore 清單）**改動前 327/327、改動後 334/334 全過**（新增 7 支：6 支本議題 + 1 支時代說明廣播）。正式 browser proof：新增 `/test/setup-era-restrict-ignore-distance-proof`（`server/main.py`）與 `scripts/validate_era_restrict_ignore_distance.py`（本議題與下一則 P1 共用，**7/7 全過**），以真實瀏覽器用手牌的「行動」按鈕正常打出「思想家」後，從前端權威狀態 `window.lastGameState.pending_choice` 讀出 `card_build_organization` 候選城鎮，五個情境全部相符——反賊時代生效：38 個候選＝牆內只有南京/杭州（距上海 1 格）＋牆外 36 個（維持無視距離）；同上但 `build_range_bonus=1`：43 個＝牆內 7 個（2 格內）＋牆外 36 個；時代未觸發對照組：77 個＝牆內 41 個（整個發展空間，無視距離）＋牆外 36 個；哈薩克時代生效（哈薩克玩家、烏魯木齊）：49 個＝牆內只有奇臺/昌吉＋牆外 47 個；反賊時代生效但玩家是哈薩克陣營（不屬 target_camp）：59 個＝牆內 12 個（遠超 1 格，完全不受影響）＋牆外 47 個。東洋奧援 tier-3 降級也在同一支 validator 內以兩個時代各自對照驗證。證據與截圖在 `docs/records/era-restrict-ignore-distance/`。

### P1：時代關卡達成畫面的縮小鍵在浮窗左下角，且縮小後任何玩家都無法再點開說明
- [done] 使用者需求：「我認為時代關卡達成畫面的取消按鈕不要放在左下角。要可以讓各個玩家再次點擊，看到時代關卡的說明。」（2026-08-09 使用者提出）
  - **按鈕位置——使用者是對的，且指的是浮窗自己的左下角**：`static/index.html` 的 `#eraAchievementModal` 只有一顆按鈕 `#eraAchievementMinimizeBtn`（文字「縮到右上角」），放在浮窗最下方的 `.era-achievement-actions` 區塊裡，而 `.modal-choice-btn` 沒有設寬度，所以它是一顆靠左的 inline-block 按鈕。實測量測（1500×950 瀏覽器）確認修改前它在浮窗內的相對位置為 left 2.5%／top 91%，也就是**浮窗左下角**（截圖 `before_1_modal_full.png`）。註：`#eraPinnedNotice`（縮小後的釘選卡片）本來就在畫面右上角，跟按鈕是兩個不同元素，只看 CSS 會誤判成「本來就在右上角」。
  - **無法再次點開——確認為真**：`minimizeEraAchievement()` → `closeEraAchievementModal()` 只做 `overlay.style.display='none'`，沒有保留任何可重開的入口；縮小後的表現 `#eraPinnedNotice` 裡的 `.era-pin-card` 只是純 `innerHTML` 的靜態 `div`，**從來沒有綁過任何 click handler**。加上 `renderEraAchievement()` 的自動彈出只在 `lastEraNotificationKey` 變動時觸發一次，導致縮小後（或對於根本沒看到彈窗的其他玩家）在該時代整段生效期間都再也叫不出達成條件／紅軍壓制／革命反撲的說明。
  - **修法**：①`static/index.html` 把按鈕搬進新的 `.era-achievement-header`（與「時代關卡達成」標題同一列、`justify-content: space-between`），`static/style.css` 給它 `position: sticky; top: -18px`＋不透明底色，讓浮窗內容需要捲動時按鈕仍固定在**右上角**（與釘選卡片的位置慣例一致，也符合按鈕自己的文字「縮到右上角」）；浮窗底部改放繁中提示「縮小後可隨時點右上角的時代關卡卡片，重新叫出完整說明。」②`static/app.js` 把浮窗填值抽成 `fillEraAchievementModal(info)`，新增全域 `openEraAchievementById(eraId)`，並把 `.era-pin-card` 從 `div` 改成可點的 `button`（加上 hover/focus 樣式、`title` 與「點擊查看完整說明」提示字），`onclick` 直接重開該時代的完整浮窗；新增 `eraAchievementViewId` 記住浮窗目前顯示的是哪個時代，避免玩家手動點開時代 A 之後被最新的時代 B 通知蓋掉內容；`era_notification` 已結束但仍有其他生效中時代時，釘選卡片改為繼續保留可點。③`server/game.py` `state()` 把 `active_era_details` 每一筆補上 `_era_notification_payload()` 的達成條件／紅軍壓制／革命反撲／期限文字——原本只有 `era_notification` 那一筆才有，其他生效中的時代點開會沒有內容可顯示。這三份資料本來就是全域廣播（不是只給觸發者），已用測試確認。
  - 驗證：完整 pytest（同 baseline ignore 清單）**334/334 全過**，含新增的 `test_active_era_details_carry_full_explanation_for_every_viewer`（以觸發方與紅軍兩個 viewer 分別取 `state()`，確認 `active_era_details` 都帶齊四段文字）。正式 browser proof `scripts/validate_era_restrict_ignore_distance.py`（與上一則 P0 共用，**7/7 全過**，1500×950、反賊時代生效）：修改前實測按鈕位於浮窗 left 2.5%／top 91%（**左下角**，證實使用者說的是浮窗自己的左下角）、點擊釘選卡片後 `modal.display` 仍為 `none`（兩位玩家皆然）；修改後按鈕位於 left 85.3%／top 2.4%（**右上角**），觸發方點釘選卡片後 `modal.display='flex'` 且標題正確（`docs/records/era-restrict-ignore-distance/era-modal-reopened-by-trigger-player.png`），**從未看過彈窗的另一位玩家（紅軍）**點同一張釘選卡片同樣正確叫出完整說明、且達成條件與紅軍壓制欄位都不是「暫缺」（`era-modal-reopened-by-second-player.png`）。另在 1100×620 短視窗捲到底確認 sticky 表頭讓按鈕仍留在右上角可見。

### P0：安全屋是被動能力，卻被做成「自己一組建立組織的按鈕／面板／地圖捷徑」（上一輪修法沒修到真正的層次）
- [done] 使用者 playtest 回報（第 2 輪、退回上一輪修法）：「完全不對啊！安全屋的功能，只有建立組織的時候才會發動啦～遊戲剛開始，根本不能建立組織，根本不應該有安全屋的按鈕可以用」。第 1 輪已回報「遊戲一開始就出現一個安全屋的按鈕要我選擇在哪建立組織，根本本末倒置」。（2026-08-09 使用者 playtest 回報並要求修正）
  - **根因（上一輪只修到距離算式，沒修到「這個 UI 根本不該存在」）**：安全屋在 `data/factions/all_faction.integrated.v2.json` 的型別是 `"type": "passive"`，對照同一份資料裡的 `"activated"`（賭徒耳語、立場試探——這些才該有自己的按鈕，由 `renderFactionActionPanel()` 產生）與 `"triggered"`（商貿組織，自動搭在別的動作上、完全沒有按鈕）。被動能力的唯一正確表現方式，是**默默修改玩家本來就在做的另一個動作的結果**；安全屋就是「玩家用正常方式（打出帶 `build` 效果的行動卡）建立組織時，牆內目標的可建立距離 +1」。但實作把它做成了一個獨立的、從第 1 回合就能點的「選起點→選目標→免出牌直接建組織」動作，共有三個入口：①`static/app.js` 的 `renderBuildSupport()` ＋ `static/index.html` 的 `#buildSupportPanel`「支援建立」側欄，顯示條件只有 `isSafehouse && 行動階段 && 輪到自己`，開局沒出任何牌就會出現；②`static/leaflet_game_map_logic.js` 的 `playerHasSafehouse()`／`buildOptionsForTown()`，點自己的城鎮就額外亮出建立目標、再點一下就直接送出建組織；③兩者都打到 `server/main.py` `"build"` 動作的 `from` 分支 → `Game.build_organization_with_support()`。對照 `data/turn_flow.v1.1.json` 的行動階段規則（「打出手牌結算效果」或「把手牌當資源購買」），**根本沒有「免出牌直接建組織」這種 board action**；`build_organization_with_support()` 開頭就 `_pending_board_action_error()` 擋掉待決選擇，代表它從設計上就只服務「無卡牌的自由動作」這條不存在的路徑，而且前端只有安全屋玩家看得到它——所以它整條就是這個 bug 的產物，不是通用機制（上一輪把它誤判為「既有、任何玩家都能用的正常 board action」，這是錯的）。
  - **修法**：①移除 `static/app.js` 的 `renderBuildSupport()`、`pendingBuildOrigin` 與其呼叫點，並從 `static/index.html` 移除 `#buildSupportPanel`／`#buildSupportInfo`／`#buildSupportOrigins`／`#buildSupportTargets` 整塊「支援建立」面板；②移除 `static/leaflet_game_map_logic.js` 的 `playerHasSafehouse()`／`buildOptionsForTown()`／`isInsideWallTown()`／`selectedBuildTargets`／`buildHighlightLayer`／`resetBuildSelection()`，以及 `renderMovementHighlights()` 內的安全屋高亮、marker click handler 裡「點高亮目標直接送建組織」的分支、`focusSelectedTown()` 的建立目標入鏡、狀態列兩處安全屋提示文字（連同已無人使用的 `townHasAnyOrganization()`）；③`server/main.py` 的 `"build"` 動作移除 `from` 分支，一律走 `game.build_organization()`（它會先把卡牌／奧援／事件／年代效果產生的待決建立選擇在該城鎮結算，是地圖「在目前城鎮建立組織」按鈕的正規路徑）；④`server/game.py` `build_organization_with_support()` 移除安全屋距離加成並補上繁中 docstring 說明它已不是玩家可操作的動作——**保留這個方法**是因為 `scripts/validate_era_effects_runtime.py`、`scripts/validate_event_cards_runtime.py`、`scripts/validate_enemy_occupancy_rules.py`、`scripts/tests/test_pending_choice_board_action_guards.py` 都拿它當「在距離外佈一個組織」的佈局工具，刪掉會連累這些與本 bug 無關的既有 proof；它現在沒有任何前端或 WebSocket 入口。⑤`_card_build_town_choices()`（卡牌建立候選）與 `_interactive_support_build_towns()`（奧援互動建立候選）維持原樣不動——這兩處才是安全屋唯一正確的生效點。共用組織機制（`_shared_origin_owner`／`_town_has_shared_org_access`，供移動、瓦解、勝利條件計數使用）完全未動。前端 `?v=` 快取字串同步換成 `safehouse-passive-no-button-20260809`。
  - 驗證：完整 pytest（同 baseline ignore 清單）**改動前 327/327、改動後 327/327 全過**。重寫 `scripts/validate_safehouse_range_frontend.py`（原本斷言「支援建立」面板要列出特定目標，是針對上一輪錯誤修法寫的，已整份改掉）為 **4/4 全過**：①②以臺北／香港城兩個安全屋根據地開局（行動階段、手牌 0、未出任何牌），真實瀏覽器確認 `#buildSupportPanel` 等 DOM 完全不存在、`renderBuildSupport` 未定義、畫面上沒有任何含「安全屋」或「支援建立」字樣的可點擊元素（截圖 `docs/records/safehouse/safehouse-no-button-at-game-start-臺北.png`、`safehouse-no-button-at-game-start-香港城.png`）；③戰略地圖 iframe 內確認 `playerHasSafehouse`／`buildOptionsForTown`／`selectedBuildTargets`／`buildHighlightLayer` 皆已不存在，選取香港城後沒有任何建立高亮，直接點 2 格外的牆內城鎮（廣州）不會建出任何組織（截圖 `safehouse-map-no-shortcut.png`）；④**正面案例**：同一位香港玩家手上發一張「組織經驗丙」（效果就是 `{"type":"build","range":1}`），用手牌的「行動」按鈕正常打出後，後端候選城鎮清單為 `九龍城/柴灣/澳門/赤柱`（1 格內牆內）＋`廣州/沙田/油尖旺/葵青/西貢/觀塘/赤臘角`（**2 格外牆內，安全屋 +1 才進得來**），並在地圖上實際點廣州＋「在目前城鎮建立組織」完成建立，證明能力沒有被刪掉、只是回到唯一正確的觸發路徑（截圖 `safehouse-card-build-choice-includes-two-step-inner.png`）。`/test/setup-safehouse-range-proof` 新增選填 `card` 欄位以支援這個正面案例。既有 `scripts/validate_safehouse_build_range.py`、`scripts/validate_era_effects_runtime.py`（15/15）、`scripts/validate_enemy_occupancy_rules.py`（5/5）重跑通過；`scripts/validate_event_cards_runtime.py` 有一項 `test_event_red_dissolve_ui_reuses_target_map_highlight` 失敗，經 `git show HEAD:static/app.js` 比對確認該 needle（`targetChoicesWithMapHighlight = new Set(`）在改動前的 app.js 也不存在，屬**既有 pre-existing 失敗、與本次改動無關**；該檔真正跟建組織有關的 `test_tibet_border_build_wall_org_grants_two_moves` 單獨重跑通過。
...
- [done] 使用者 playtest 回報：打出行動卡「組織經驗丙」，在戰略地圖上可以正常選到「廈門」，左側「建立組織」按鈕也確實亮起可按，但按下去完全沒有建立組織，也沒有出現任何錯誤訊息。（2026-08-08 使用者 playtest 回報）
  - 根因：戰略地圖 iframe 自己維持一條 WebSocket（`static/leaflet_game_map_logic.js` `connectGameMap()`），但它從來沒有 `onclose`／`onerror`／重連邏輯——只要這條連線斷過一次（伺服器重啟、筆電睡眠喚醒、網路閃斷，或伺服器端 broadcast 對某個已死連線丟出例外而把本連線一起關掉），它就永遠停在 `CLOSED`。此時父頁 `static/app.js` 有自己的 `scheduleReconnect()` 會自動重連，所以指揮中心看起來一切正常，pending choice 也照樣以 postMessage 打進地圖 → 候選城鎮亮著、`refreshDirectBuildUi()` 也照樣把按鈕設成 enabled；但真正送出動作的 `sendDirectBuildAction()`（`static/leaflet_game_map_logic.js`）在 socket 未開時只是 `return { ok: false, reason: 'socket-not-open' }`，按鈕的 click handler 完全忽略回傳值，於是變成「按鈕亮著、按下去靜默失效、沒有任何錯誤」。`sendMoveAction()`／`sendDissolveAction()`／安全屋建設也有同一個靜默失效問題。**與廈門這座城鎮本身無關**，也不是 `2cbbeff`／`b32d742` 造成的回歸（那兩個 commit 完全沒有碰地圖連線或建立組織路徑）；後端 `_card_build_town_choices()`／`_resolve_town_choice()` 實測完全正常。伺服器端另有兩個會把地圖那條 socket 弄死／踢掉登記的成因：`broadcast_game_state()` 對已死連線 `send_json` 拋出的例外會冒到「正在送出動作那位玩家」的 handler，把對方的 socket 一起關掉；`remove_connection()` 無條件 pop 該玩家的登記，會把同一位玩家後來建立的新連線一起踢掉，而 `register_connection()` 在滿桌 4 人時連同一位玩家的重連也一併拒絕。
  - 修法：`static/leaflet_game_map_logic.js` 把連線邏輯抽成 `openMapSocket()`，補上 `onopen`／`onclose`／`onerror` 與指數退避的 `scheduleMapSocketReconnect()`，並比照父頁綁定 `online`／`pageshow`／`visibilitychange` 立即重連；新增守門 `requireOpenMapSocket()`，`sendDirectBuildAction()`／`sendMoveAction()`／`sendDissolveAction()`／安全屋建設在連線中斷時改為顯示繁中錯誤「與伺服器的連線已中斷，正在自動重新連線；連上後請再按一次。」並立刻觸發重連，不再靜默失效；重連成功時自動清掉該提示。`static/leaflet_game_map.html` 同步把 `?v=` 換成 `map-socket-reconnect-20260808`。伺服器端 `server/main.py` `broadcast_game_state()` 改成逐一連線 try/except，單一已死連線只會被清掉、不會把別人的 socket 連帶關閉；`server/game_manager.py` `remove_connection()` 加上 `websocket` 參數（只有目前登記的就是這條連線才移除），`register_connection()` 的 4 人上限只限制「不同玩家」，同一位玩家的重連永遠允許。
  - 驗證：新增 `scripts/tests/test_map_socket_resilience.py`（4 支：滿桌重連仍可登記、舊連線收尾不會踢掉新連線、broadcast 遇到死連線仍送達存活連線且只清掉死的、前端守門靜態稽核）。完整 pytest（同 baseline ignore 清單）**327/327 全過**（baseline 323 +4）。新增正式 browser proof `scripts/validate_map_socket_reconnect_browser.py`：打出組織經驗丙 → 兩條 WebSocket 同時斷線 → 地圖自動重連 → 在正式 Leaflet 地圖點選廈門 → 按「在目前城鎮建立組織（效果）」→ 權威盤面確實在廈門長出組織，**7/7 全過**、console 0 errors，證據與截圖在 `docs/records/map-ui/socket-reconnect/`。同一支 proof 對修改前的 `leaflet_game_map_logic.js` 重跑會掉到 **3/7**，且失敗樣態與使用者回報完全一致：`build_button_arms_for_xiamen_after_reconnect` PASS（按鈕確實亮著、文字為「在目前城鎮建立組織（效果）」）但 `authoritative_state_shows_the_organization_in_xiamen` FAIL（組織仍只有福州）。
...
- [done] 使用者要求充分檢查英美、東洋、南洋、印度、天方、歐洲、北國、臺灣、紅軍全部奧援，確認是否還有與「東洋奧援建立後漏掉共用 hook」相似的問題。（2026-08-07）
  ...
  - **2026-08-07 後續裁決｜紅軍奧援資源模式**：使用者確認紅軍奧援應可用「資源」方式取得 **1資金＋1宣傳**，覆蓋 2026-06-07 對一般奧援「資源模式只棄置」的舊裁決；紅軍奧援是唯一例外。Runtime 將「印刷資源 1/1」與「起始牌購買費用 0/0」分離：資源模式不抽牌並直接進使用者自己的棄牌堆；行動模式才抽牌並依陣營流轉（紅軍使用後選擇任一反共玩家棄牌堆，反共玩家使用後回紅軍棄牌堆）。正式 UI 第一顆按鈕由「棄置」改為「資源」，普通奧援維持「棄置」。新增紅軍／反共 × 行動／資源四象限測試與正式 Browser proof。
  - **2026-08-11 卡面設計修正**：使用者指出紅軍奧援的玩法更接近一般行動卡，不應沿用普通奧援的三級條件版型。卡面已改用一般行動卡資訊架構：上方標示「起始牌」、中段主插圖、單一「卡牌效果」框明確標示只有行動模式會抽牌並進行陣營間流轉、底列以標準資源圓章呈現 **1資金＋1宣傳**；保留紅軍專屬紅色識別。前端為該圖片加入獨立 cache bust，正式 Browser proof 驗證 1100×1350 圖片完整載入、未裁切、按鈕仍為「資源／行動」、console 0 errors，**5/5 passed**；資源模式現行語意 Browser regression **5/5 passed**，完整 pytest **334/334 passed**。證據位於 `docs/records/support-cards/red-support-action-art/`。
...
- [done] 使用者需求：最後結算畫面，應該要直接顯示牆內和牆外組織數量。我需要華麗一點的畫面，比如說，紅軍贏了，其他陣營生靈塗炭；台灣綠線陣營贏了，台灣人民自由民主進步富裕。台灣藍線陣營贏了，開始規劃反攻大陸。蒙古贏了，連內蒙古都囊括其中。諸如此類的東西。（2026-08-06 使用者提出並要求分兩塊處理）
  ...
  - **(1) 前端顯示（`static/app.js` `renderVictoryModal`／`static/style.css` `.victory-summary-row`）**：戰況表從原本 5 欄（玩家/陣營/組織/資金/宣傳）擴充成 7 欄，插入「牆內」「牆外」兩欄，直接讀 `p.organization_counts.inside_wall`／`.outside_wall`（缺欄位時保底用 `total - inside_wall` 換算，不會顯示負數）；`.victory-glass` 加寬到 660px 容納新欄位。
  - **(2) 結局敘事（`static/app.js` 新增 `VICTORY_ENDING_NARRATIVES` + `victoryEndingNarrative()`）**：陣營先歸類再套模板，複用既有的 `factionCategoryOf()`（與 `/factions` 後端回傳的 category 完全對應：`red_army`/`taiwan`/`hong_kong`/`uyghur`/`tibet`/`manchuria`/`mongol`/`kazakh`/`rebel` 共 9 類）——`taiwan_green`／`taiwan_blue` 用完整 faction_id 各自寫一段（因為使用者明確要綠線/藍線兩種不同敘事），其餘 7 類（含西藏 3 個流亡據點、維吾爾 4 個海外據點共用各自 1 段）用類別鍵。敘事文字直接把彙總後的 `insideWallTotal`／`outsideWallTotal`（`players.reduce()` 加總全體玩家的 `organization_counts`）寫進句子本身，不只是列在表格裡——例如紅軍獲勝敘事會寫「牆內組織僅剩 N 個苟延殘喘」，蒙古獲勝敘事會寫「連內蒙古都一併囊括其中」呼應使用者原文用詞。敘事區塊（`#victoryEnding`，新增於 `index.html`）用左側色條＋標題色跟著 `winnerColor`（陣營代表色）走，視覺上與獲勝陣營一致。
    - **使用者中途回饋並簡化**：原本規劃反賊分類底下 18 個地方勢力（滇/苗/傣/晉/齊/吳越/粵…等）比照西藏/維吾爾模式各自帶入陣營名稱寫一段「{陣營}獨立建國」；使用者中途表示「反賊做一個版本就好，不必個別做，可以先做一個反賊大團結之類的版本」，改為統一一段「烽火燎原，反賊大團結」敘事，以實際獲勝的地方勢力為號召者但強調「這是屬於全體反賊的勝利，不是任何一方的獨吞」，不用再幫 18 個地方勢力各寫一段。
  - 驗證：Playwright 對真實瀏覽器——(a) 直接呼叫 `victoryEndingNarrative()` 對 11 個代表性 faction_id（含西藏/維吾爾的不同據點變體、2 個反賊子陣營）驗證都能正確解析、關鍵字比對（紅軍「生靈塗炭」、綠線「自由」+「繁榮」、藍線「反攻大陸」、蒙古「內蒙古」）、西藏/維吾爾不同據點共用同一段、反賊 2 個子陣營共用同一段標題文字、牆內外數字有正確帶入文字；(b) 用既有 `/test/setup-victory-proof` 測試端點實際跑兩場對局（綠線獲勝、紅軍獲勝），確認戰況表牆內/牆外欄位數字正確（GREEN 牆內0牆外5、RED 牆內7牆外0，對應臺北/桃園在牆外、北京/天津在牆內）、結局敘事標題與內文正確顯示、邊框強調色跟著陣營色切換。共 13 項檢查全過。另外重跑既有 `scripts/validate_victory_screen_and_creator_name.py`（5/5，確認新增的表格欄位沒有打壞既有的行數/標題/縮小徽章斷言）。完整 pytest（同 baseline ignore 清單）**280/280 全過**（無變動，純前端新增功能，`organization_counts` 欄位是既有後端資料）。
  - **2026-08-06 使用者追加需求並補做：「每個陣營都要有自己的畫面」**：使用者明確要求結局敘事不能只有文字，要有畫面、要有圖片，且臺灣綠線/藍線要分開呈現。
    - **稽核圖片生成能力**：檢查 `scripts/generate_all_era_card_trials_ai_art.py`、`generate_all_action_card_ai_art.py` 等既有產圖腳本，發現它們依賴 `importlib.import_module("plugins.image_gen.openai-codex")` 這個 provider；實際在這個環境整台機器搜尋、也直接嘗試 import，確認 `plugins` 套件完全不存在（`ModuleNotFoundError: No module named 'plugins'`）。從模組命名判斷，這應該是先前透過另一套工具（OpenAI Codex CLI，非本次 session 使用的 Claude Code）產圖時才有的外掛，這個環境無法呼叫，因此無法直接產生新插畫。`static/card-art/era/` 底下雖然已有各陣營的 AI 插畫（如 `[香港]香港人被自殺.png`、`[藏國]藏國騷亂.png`），但那些是時代關卡「壓制/危機」情境的插圖，語意上與「該陣營獲勝、局勢大好」的結局畫面相反，直接借用會誤導玩家（例如藏國勝利卻配一張「藏國騷亂」的圖），因此沒有重用。
    - **與使用者確認方向**：用 `AskUserQuestion` 提出三個選項（純 CSS/SVG 風格化場景／使用者自己拿外部工具產圖再交給我接上／先做 CSS/SVG、之後可覆蓋換成真圖），使用者選擇「先用 CSS/SVG 做風格化場景」。
    - **實作**（`static/index.html`／`static/style.css`／`static/app.js`）：`#victoryEnding` 內新增 `#victoryEndingScene` 疊層結構（`-sky`／`-sun`／`-ground`／`-texture` 四個絕對定位子層，高度 130px，位於敘事文字上方、卡片圓角範圍內），純 CSS 疊層組成漸層天空＋剪影地形（`clip-path: polygon(...)`）＋光源圓點，不是真的插畫但每個陣營都有自己專屬的一組配色與地形剪影：紅軍＝暗紅鐵幕＋鋸齒圍牆剪影＋鐵絲網紋理；臺灣綠線＝暖色日出＋城市天際線；臺灣藍線＝深藍轉暮紅＋遠方山稜（呼應「對岸」）＋紅色落日；香港＝晨曦粉金＋維港天際線；維吾爾＝湖水綠松石天空＋沙丘與圓頂剪影；西藏＝酒紅轉金＋雪峰鋸齒剪影；滿洲＝淡金晨光＋森林山脈剪影；蒙古＝遼闊藍天大太陽＋起伏草原剪影；哈薩克＝藍調河谷＋山稜＋河面反光紋理；反賊＝赤陶暖天＋破碎多峰剪影＋多面小旗紋理（呼應「反賊大團結」的多方勢力感）。`victoryEndingNarrative()` 回傳值新增 `sceneKey`（與敘事模板同一套查找邏輯：先比對完整 faction_id，找不到才退回 `factionCategoryOf()` 分類），`renderVictoryModal()` 依 `sceneKey` 動態切換 `#victoryEndingScene` 的 class（`victory-ending-scene--{sceneKey}`）套用對應場景樣式。架構上刻意讓場景層與文字層分離、獨立一個容器，之後若透過外部工具產出真的插畫，只需要在 `.victory-ending-scene` 疊一張 `<img>`／`background-image` 覆蓋，不需要更動任何邏輯或結構。
    - 驗證：Playwright 對真實瀏覽器直接呼叫 `renderVictoryModal()` 灌入 10 種陣營（含西藏/維吾爾其中一個據點、反賊其中一個子陣營）的假資料，確認每一種都正確套上對應的 `victory-ending-scene--{key}` class（10/10 全過）；並對 6 個代表性陣營（紅軍/綠線/藍線/蒙古/西藏/反賊）截圖確認場景視覺上彼此明顯不同、與各自的敘事語氣相符。完整 pytest 280/280、既有兩支勝利畫面 validator（5/5、13/13）皆無回歸。
  - **2026-08-06 使用者推翻 CSS/SVG 暫代方案，要求改用 AI 插畫且採全版畫面；已完成**：確認目前 Hermes 內建的 `plugins.image_gen.openai-codex` 可透過 Codex OAuth 使用 OpenAI Responses `image_generation`，以 `gpt-image-2-medium` 為 10 種結局場景各產生一張 1672×941 的 16:9 無 UI 插畫：紅軍、臺灣綠線、臺灣藍線、香港、維吾爾、西藏、滿洲、蒙古、哈薩克、反賊。成品與完整 prompt manifest 放在 `static/victory-art/`，可續跑腳本為 `scripts/generate_victory_ending_ai_art.py`；全部逐張人工檢查過構圖、人物、地標、裁切及明顯破圖，總覽位於 `docs/records/playtest-flow/victory-ai-art/victory_ai_art_contact_sheet.png`。
    - **全版整合**（`static/index.html`／`static/style.css`／`static/app.js`）：AI 圖片改為 `#victoryModal` 最底層的 1280×720 全舞台 `object-fit:cover` 背景，不再是敘事卡裡 130px 高的小橫幅；結果資訊改成舞台底部橫向毛玻璃板，左欄放勝利者與客製結局敘事，右欄放牆內／牆外／資源戰況表，上方保留大面積電影式插畫。圖片載入失敗時保留深色漸層 fallback；切換陣營時依 `sceneKey` 換圖，並以版本 query 避免舊快取。
    - **正式驗證**：新增 `scripts/validate_victory_fullscreen_ai_art.py`，逐一檢查 10 張 runtime PNG 尺寸、10 種陣營的正式 browser 全舞台覆蓋、場景 class、圖片載入、底部資訊板邊界與 console，**21/21 passed**；既有 `validate_victory_screen_and_creator_name.py` **5/5 passed**；完整可收集 pytest（既有 4 個會在 collection 直接跑外部 WS/Playwright/Lobby 流程的檔案依既定方式 ignore）**280/280 passed**。正式綠線全版截圖已更新 `docs/records/playtest-flow/victory_screen.png`，10 種實際 UI 截圖及驗證紀錄位於 `docs/records/playtest-flow/victory-ai-art/`。
  - **2026-08-06 使用者驗收後要求紅軍結局必須更明顯「生靈塗炭」；已重製**：第一版紅軍圖偏向整齊的威權閱兵廣場，雖陰森但仍有「盛大勝利」感，不夠呈現一般人的災難。重新強化 OpenAI 產圖 prompt，改為燒毀公寓、坍塌商店、斷電城市、廢棄大眾運輸、工業黑煙、軍事檢查哨與探照燈；前景明確放入老人、抱著孩子的父母、拖著僅存家當的流離失所者，在泥濘與灰燼中排成長隊，禁止血腥、屍體與正在發生的暴力，但必須讓飢餓、悲痛、恐懼與無家可歸一眼可見。為避免底部結算板遮住最關鍵的難民隊伍，紅軍專屬畫面把資訊板移到上緣並略微提高透明度，其他 9 種陣營仍維持底部資訊板。生成腳本新增 `--scene`／`--force`，可只重生指定場景而不覆蓋其他成品；圖片 cache 版本同步升到 `ai-v2`。正式 `winner='red_army'` browser proof 另立真實 fixture，會先關閉可能遮擋的事件卡 Zoom-in，再驗證 `RED 獲勝`、紅軍場景 class 與上緣資訊板，聚焦驗證更新為 **22/22 passed**；最終截圖為 `docs/records/playtest-flow/victory-ai-art/victory_red_army.png`。
  - **2026-08-06 使用者進一步要求紅軍勝利依觀看陣營呈現兩種主觀版本；已完成**：同一場 `winner='red_army'` 對局，前端以目前 viewer 的 `playerId` 對應 `state.players[].faction` 分流。非紅軍玩家維持上述 `red_army.png` 生靈塗炭版、上緣資訊板與「紅色鐵幕，籠罩天下」敘事；紅軍玩家改看新增的 `red_army_triumph.png` 征服世界宣傳版（巨型地球儀、跨洲城市地標、全球艦隊、戰機、無盡軍陣與赤旗）、底部資訊板與「赤旗遍寰宇，天下歸一統」敘事。這是 viewer-scoped 純前端投影，不改 winner 或公開遊戲狀態。圖片 cache 版本升到 `ai-v3`；正式 fixture 分別以 `player_id`（綠線 viewer）與 `red_player_id`（紅軍 viewer）連入同一場紅軍勝局，驗證兩張圖片、兩套敘事與上下資訊板正確分流；全部 11 張 runtime AI 圖與 10 陣營矩陣合計 **24/24 passed**，兩張正式截圖為 `victory_red_army.png` 與 `victory_red_army_triumph.png`。
  - **2026-08-06 點擊勝利插畫可查看完整原圖；已完成**：正常結算畫面繼續以 `object-fit: cover` 保持全版電影式構圖；玩家點擊未被結算板覆蓋的圖片區，或以鍵盤 focus 後按 Enter／Space，會開啟 `role="dialog"`、`aria-modal="true"` 的獨立全螢幕檢視器。檢視器直接載入當前 viewer-scoped 原圖，以 `object-fit: contain` 顯示完整 16:9 畫面，不再被結果板遮住或裁切；可按 Esc、點黑色背景或右上角 × 關閉，關閉後焦點回到圖片，點結果資訊板不會誤開，Tab 焦點會留在檢視器的關閉按鈕。桌面與窄 viewport 均保留安全邊距及操作提示。正式 browser proof 同時覆蓋紅軍／非紅軍兩個版本、完整原圖幾何比例、三種關閉方式、鍵盤操作、焦點限制與恢復、結果板 propagation 與 console，更新為 **31/31 passed**；完整圖證據為 `victory_red_army_full_art.png` 與 `victory_red_army_triumph_full_art.png`。
...
- [done] 使用者需求：當某一玩家使用能力和卡牌時，其他玩家應該也要直接跳出視窗，看到其他人正在做什麼，使用什麼能力和卡牌，卡牌的圖片要秀出來。不想看可以縮到左下角，變一個小視窗。（2026-08-06 使用者提出並要求處理）
  ...
  - **2026-08-06 第二輪回饋並改版（拿掉徽章、改成真正的置中大彈窗）**：使用者看過第二版截圖後表示第一輪修正後的「展開」狀態還是太小、其實比較像縮小狀態，且明確表示不需要 📣 圓形徽章這個第三種狀態；使用者要的「展開」是會直接擋住畫面一大塊、營造「像看轉播一樣看到其他玩家在做什麼」的既視感。
    - 根因：把「展開」設計成固定在左下角的一張卡片（即使加大到420px），本質上仍是個「角落通知」，不是「跳出視窗」；而圓形徽章又是第三種獨立狀態，讓「縮小」的定義變得不一致（使用者認為既有的角落卡片本身就該直接當「縮小」狀態使用，不需要再縮一層）。
    - 改版方向：拿掉圓形徽章這個獨立元素，`#peerActionNotice` 改成單一元素的兩態切換——展開＝真正的全螢幕 `position:fixed; inset:0` 疊層（暗化＋模糊背景、`justify-content:center`，內部包一個 `.peer-action-notice-card`，寬度 `min(640px, 100vw-48px)`，卡牌圖片放大到180px、字級全面加大，比照既有 `.modal-overlay`/`.modal-glass` 全螢幕彈窗語彙）；縮小＝同一個元素疊加 `.peer-action-notice-minimized` class，改寫成 `inset:auto; left:24px; bottom:24px; width:auto; height:auto`（fixed 定位下配合 `width/height:auto` 會自動收縮回內容大小，不會殘留一個佔滿全螢幕的隱形點擊阻擋層），並透過巢狀選擇器把卡片本身、圖片、文字全部等比縮小成一張左下角小卡片——縮小狀態下背景不再暗化，可以正常操作畫面其他地方。縮小鈕改成雙向切換（`—`↔`⤢`），不再需要另一個獨立的展開按鈕元素。
  ...
  - **2026-08-06 回合交接時自動關閉通知；已完成**：當 `state.current_player` 變成目前 viewer 自己時，`renderPeerActionNotice()` 會立即關閉其他玩家的通知，不論通知仍是置中大型彈窗或已縮成左下角小卡片；同時清除 minimized/content 狀態並保存當前 `action_log` 快照，避免之後再次進入等待狀態時重播上一回合的舊通知。換局時仍會精確重設 log 游標，不受上一局較長歷史影響。新增 `scripts/validate_peer_action_notice_turn_handoff.py` 正式 browser proof，覆蓋：其他玩家新動作正常展開、可縮小、本人回合關閉縮小通知、舊通知不重播、下一則新動作仍會展開、本人回合也會關閉未縮小的大型通知、console 無錯誤，**7/7 passed**。前後截圖及 JSON 位於 `docs/records/playtest-flow/peer-action-notice-turn-handoff/`；完整 pytest 首輪碰到既有隨機事件 fixture 使 `test_business_network_ws_result.py` 暫時進入 `event_build_organization`，單測立即重跑 **1/1 passed**，隨後完整重跑 **280/280 passed**。
  - **2026-08-06 其他玩家通知全面繁體中文化；已完成**：通知不再直接顯示後端混合語言 action log（例如 `RED played 合作談判`），新增 `localizePeerActionNoticeText()` 將常見的使用卡牌、作為資源、反應取消、發動能力、抽牌、取得資源、建立／移動／瓦解組織、購買、棄牌與奧援結算等句型轉為自然繁中；道路／鐵路等 mode 也會中文化。玩家自行輸入的英文名稱（例如 `RED`／`GREEN`）原樣保留；若遇到尚未收錄且仍含英文系統詞的句型，安全退回「玩家名 完成了一項行動」，不讓 backend 英文洩漏到通知 UI。正式 browser proof 改以真實英文輸入 `RED played 合作談判` 驗證畫面輸出 `RED 使用了合作談判`，另跑 11 組代表句型矩陣並移除玩家名後檢查零英文系統詞；連同回合交接共 **8/8 passed**，既有全域玩家訊息繁中 browser regression **10/10 passed**、完整 pytest **280/280 passed**，截圖已更新於同一紀錄目錄。
  - **2026-08-06 多回合後通知消失根因修復；已完成**：根因不是 timeout 或縮小狀態，而是 `server/game.py` 將 `action_log` 限制為最近 100 筆；到達上限後每新增一筆就移除最舊一筆，陣列長度持續為 100。舊前端只用 `entries.length > peerActionNoticeSeenLogLength` 判斷新消息，因此到第 100 筆後便永遠停止偵測。改以 `peerActionNoticeLogSnapshot` 保存上一份 log，`peerActionNoticeNewEntryStart()` 尋找「舊快照 suffix／新快照 prefix」的最長重疊區，正確識別固定長度滑動視窗尾端的新項目；完全相同 state 不重播、換局／本人回合仍正確消耗快照。正式 browser proof 新增 100 筆滿載後同長度滑動、相同 state 去重、連續 30 次滿載滑動（交替重複使用同卡）的壓力情境，以及 390×844 手機 viewport 中滿載滑動後通知、繁中卡名、實際圖片載入與視窗邊界；連同既有圖片、縮小、回合交接及繁中檢查共 **12/12 passed**；另重跑完整 pytest **280/280 passed**、全域繁中 browser **10/10 passed**、勝利畫面與完整插畫 viewer **31/31 passed**，console、JS syntax、Python compile 與 `git diff --check` 均通過。
...
- [done] 使用者回報：烏魯木齊七五事件也是啊～～所有事件卡都應該要所有人都輪過該回合才結算。（2026-08-06 使用者回報並要求處理）
  - **範圍稽核**：全 `data/events_structured.v1.1.json` 中，`trigger.type == "end_turn_state"` 的事件**只有『烏魯木齊七五事件』一張**（`urumqi_incident`，line 46-48；沒有「（副本）」版本），其餘任務型事件全是累積計數型觸發（`play_card_with_money`/`play_card_with_propaganda`/`buy_card`/`move_organization`/`draw`/`use_faction_ability` 等）。所以本次時機修正**只影響 `end_turn_state` 這一種觸發（＝只有這一張卡）**，計數型觸發完全不動。
  - 根因：`烏魯木齊七五事件` 的 trigger 是 `{"type": "end_turn_state", "condition": "own_organization_in_scope", "scope": "牆內"}`——這是「結算當下的活狀態檢查」（`_event_state_condition_met()`，`server/game.py:477`；`_settle_current_event()` 對它有專屬的即時重新檢查分支，`server/game.py:932`），不是像其他事件那樣的累積計數。但舊碼 `advance_turn_phase()` 的 `TurnPhase.END` 分支對**所有**任務型事件一律在 `_is_final_non_red_turn_before_round_wrap()`（`server/game.py:4448`，本輪最後一位非紅軍玩家、**紅軍自己的回合之前**）就結算。對計數型觸發這是對的（紅軍行動不會推進非紅軍任務進度、失敗罰則也該落在非紅軍玩家身上、要在紅軍純行動回合之前定案）；但對活狀態檢查就有真正的漏洞：**用的是紅軍行動前的過期快照**——非紅軍玩家回合結束時牆內還有組織 → 判成功，但緊接著紅軍自己的回合把那個組織瓦解掉，整輪真正結束時條件其實已不成立，任務卻早就用舊快照定案成功了。這與本會話先前對「時代關卡觸發」「宣布勝利」兩處已確立的原則（`_detect_era_triggers`／`_check_victory` 都在 `_end_turn()` 真正 round-wrap、`current_player_index` 繞回 `round_start_player_index`、**含紅軍都行動完之後**才判定，P1：判定時機應等整輪含紅軍行動完）完全同構，但 `end_turn_state` 觸發之前沒有比照辦理。
  - 重現：以 `[非紅軍(index 0，round-start), 紅軍(index 1，最後手)]` 兩人局（紅軍必須排在非紅軍之後，行動才會落在舊結算點之後——這也是為何不能用 `make_game()`，它把紅軍排 index 0），非紅軍玩家持有 1 個牆內組織、事件為 `烏魯木齊七五事件`。舊碼在非紅軍玩家 `END` 當下（`advance_turn_phase()` 走 defer-after-refill 路徑，紅軍還沒行動）就把 `event_progress` 設為 `settled=True / status='success' / succeeded=True`；proof script 實測印出 `settled=True status=success`，證實過期快照 bug 確實存在。
  - 修法（`server/game.py:5474-5481`，`advance_turn_phase()` 的 `TurnPhase.END` 分支）：新增 `event_is_state_triggered = trigger_type == 'end_turn_state'`、`is_true_round_wrap = next_player_index == round_start_player_index`，把結算邊界改為 `settle_boundary = is_true_round_wrap if event_is_state_triggered else is_round_final_action`——即**只有 `end_turn_state` 觸發改到真正的 round-wrap 邊界（整輪含紅軍都行動完）結算，其餘觸發一律維持原本的「最後一位非紅軍玩家」邊界，一個字都沒動**。`settlement_target_player_id` 仍在 `is_round_final_action`（最後一位非紅軍）當下捕捉、存進 `event_progress` 撐到 round-wrap，所以延後結算時結算對象仍然是那位非紅軍持有者、不會變成後手就座的紅軍。實作上直接沿用既有的 `_should_defer_event_settlement_until_after_refill()` + `_end_turn()` 後的 snapshot/restore 延後結算機制（`build_organization_near_own` 成功獎勵／`discard_random` 失敗罰則本來就走 defer-after-refill，不是 `topdeck_from_discard`，不需特殊處理）：真正 round-wrap 時 `_end_turn()` 會清掉 `current_event`、snapshot/restore 分支會暫時還原本事件、對「紅軍行動後的最終狀態」重新跑 `_event_state_condition_met()` 再結算。修完後同一個 proof script：非紅軍玩家 `END` 當下 `settled=False`（正確地延後、目標已捕捉為該非紅軍玩家），等紅軍瓦解組織、整輪結束後才結算成失敗（`discard_random` 罰非紅軍持有者、手牌 5→4）。
  - 新增回歸（`scripts/tests/test_action_card_regressions.py`）：`test_urumqi_state_trigger_settles_after_red_army_turn_not_before`（重現使用者情境：非紅軍玩家 END 時事件尚未結算、目標已捕捉；紅軍瓦解牆內組織後整輪結束才判定為失敗、`discard_random` 罰非紅軍持有者），`test_urumqi_state_trigger_succeeds_when_org_survives_full_round`（對照組：紅軍未瓦解 → 整輪結束後判成功，且一樣不在最後非紅軍邊界提前判定），`test_count_based_mission_still_settles_at_final_non_red_turn`（護欄：計數型任務『北京政爭』在同樣 `[非紅軍, 紅軍最後手]` 排列下，仍在最後一位非紅軍玩家 `END`、**紅軍回合之前**就 `settled=True`，證明本次只動 `end_turn_state`、沒有連帶把計數型觸發的結算時機一起搬走）。
  - 驗證：完整 pytest（同 baseline ignore 清單）**275→278 passed, 0 failed**（恰為新增 3 支）。另跑 `scripts/validate_event_outcome_timing_audit.py` **28/28**（該稽核裡『烏魯木齊七五事件』的 success/failure 兩筆本來編碼的是舊時機，已同步更新為「整輪 round-wrap 後才結算」並保留 `東突厥集中營` 等計數型事件走原本的最後非紅軍邊界路徑不變）；`scripts/validate_turn20_victory_declaration.py`（3/3，含 `mid_round_player_victory_deferred_to_round_wrap`）、`scripts/validate_victory_rules.py`（5/5）、`scripts/validate_era_effects_runtime.py`（15/15）、`scripts/validate_era_rules.py`（5/5）、`scripts/validate_era_canonical_scope.py`（passed）全綠，確認時代觸發與勝利宣告的 round-wrap 時機完全未受影響。
...
- [done] 使用者回報：建立組織時，點選城鎮後，應該要讓建立組織的按鈕亮起來。（2026-08-05 使用者回報；同日使用者要求處理）
  ...
  - **2026-08-05 使用者要求比照辦理**：把同一套「停用變暗＋啟用金色高亮」樣式擴大套用到 `#confirmMoveBtn`（確認移動）與 `#dissolveBtn`（瓦解目前城鎮組織），選取器直接併入同一組規則（`static/leaflet_game_map.html`），視覺語彙完全一致。`#cancelMoveBtn`（取消目的地）刻意不套用高亮——它是次要／取消類操作，非「可執行的主要動作」，維持中性樣式（後續使用者確認新增「點地圖其他地方也能取消」後，此按鈕予以保留，兩者互補，見下方新項目）。透過 Playwright 對真實瀏覽器驗證確認移動按鈕：`/test/setup-enemy-occupancy-proof` 建立可移動組織 → 選取東沙（顯示2個合法移動目的地）→ 按鈕維持暗色 → 直接模擬點擊合法目的地屏東（比照真實 marker click handler 對 `pendingMoveTarget` 的賦值方式）→ 按鈕變成金色高亮，`#confirmMoveBtn.disabled` 由 `true` 變 `false`。瓦解按鈕沿用同一條 CSS 規則，未額外截圖（純選擇器擴充、無獨立邏輯分支）。完整 pytest 269/269 全過（無變動）。

### P2：點地圖其他地方應該直接取消待確認的移動目的地
- [done] 使用者需求：取消移動的做法，感覺可以直接在地圖其他任意地方點擊就取消，畫面會更簡潔，不必特地按「取消目的地」按鈕。（2026-08-05 使用者提出並確認要做）
  - 現況稽核：`map.on('click', ...)`（`static/leaflet_game_map_logic.js:1143`）本來就有「點擊空白背景（非 marker）→ `exitMovementSelection()` 全部取消」的既有行為。缺口在城鎮 marker 自己的 `click` handler（`static/leaflet_game_map_logic.js:1018`）——選好起點、地圖上顯示合法移動目的地時，點擊一個「不相關」的城鎮（不是合法目的地、玩家也無法從該城鎮行動）會直接被 `if (selectedTown && selectedMoveTargets.length) { if (canActFromTown(t.name)) {...}; return; }` 這段擋下、整個 return，等於死點擊——跟點空白背景的行為不一致，也不算「取消」。
  - 修法：把上述分支的 `else` 補上 `exitMovementSelection()`，讓「點不相關城鎮」比照「點空白背景」同樣視為取消整個選擇（`static/leaflet_game_map_logic.js:1041-1049`）。點擊「合法的另一個移動目的地」不受影響（`moveOptionForTown` 那個分支在更上面，會先攔截、直接改選新目的地）；點擊「另一個自己可行動的城鎮」也不受影響（`canActFromTown` 為真時走 `selectTownForCurrentMapAction` 改選新起點，不是取消）。`#cancelMoveBtn`（取消目的地・保留起點）按鈕本身沒有動——它是「只清目的地、保留起點」的更精確操作，跟這次「點別處＝整個取消」是互補的兩種操作，沒有移除。
  - 驗證：Playwright 對真實瀏覽器（`/test/setup-enemy-occupancy-proof`）——選取東沙（合法目的地：屏東／觀塘）→ 直接觸發合法目的地屏東的 `pendingMoveTarget`、確認 `#confirmMoveBtn` 變啟用 → 對不相關城鎮瀋陽（紅軍城鎮，非合法目的地、玩家也無法從該城鎮行動）觸發真正的 marker `click` 事件（非直接改變數值）→ 確認 `selectedTown`／`pendingMoveTarget`／`selectedMoveTargets` 全部清空、`#confirmMoveBtn` 變回停用。另外重跑「改選另一個合法目的地（觀塘）」確認仍正確改選、未被誤判成取消。純前端 JS 變更，不影響後端；完整 pytest 269/269 全過（無變動，與此改動無關）。

### P2：行動預告／行動募資使用時機（舊待查項目，已由後續裁決完成）
- [done] 這兩張卡不是「只能被動觸發，因此停用行動按鈕」。使用者後續明確裁決並已完成實作：玩家打出卡牌時立刻取得卡面資源並累積一次頂牌權利；購買完卡牌後、結束回合前可主動按「頂牌 (N)」選擇本回合購得的牌置於牌庫頂；若尚未手動處理，結束回合時系統再自動提示。多張卡可累積多次權利。完整實作與驗證記錄見前文「行動預告／行動募資的頂牌應改為玩家主動觸發」。
...
- [done] 使用者playtest回報（本 session 最高嚴重度）：到了第十一回合，事件卡抽到『上海合作組織』，當非紅軍陣營完成回合時，雙方都卡住，無法執行任何步驟。2 人局（非紅軍座位「dafdsaf」＋紅軍）。附的 action_log 顯示『上海合作組織』先成功 auto 套用過一次，之後又被重抽一次並停在 `(auto deferred)`，遊戲此後再也推不動。（2026-08-05 使用者回報，直接從回報進入調查與修正）
  ...
  - 修法（比照紅軍事件 `auto_pending` 延後到紅軍回合的既有模式；只動互動式時代的**啟動時機**，不動「偵測排在整輪 wrap」的既有規則、也不碰勝利判定時機）：
    - 新增 `_era_activation_interactive_target(era)`（`server/game.py`，緊接 `_continue_era_activation_queue` 之前）：回傳「啟動此時代會產生互動 pending_choice 的擁有者玩家」，且**僅在啟動真的會產生該 choice 時**才回傳（比照 `_start_era_red_discard_build_flow`／`_start_era_inspect_deck_top_and_reorder_flow` 的前置條件：對象有手牌／有合法建立城鎮／牌庫夠深）；不會產生互動 choice 時回傳 `None`（不延後，非互動側效果照原邊界套用）。涵蓋 `red_discard_to_build_near_target`（藏國，對象紅軍）與 `inspect_deck_top_and_reorder`（滿洲，對象滿洲陣營）。
    - 改寫 `_continue_era_activation_queue()`（`server/game.py`）的抽取迴圈：改為掃描佇列挑第一個「可立即啟動」的時代——非互動、或互動對象**就是當前玩家**——才啟動；互動對象非當前玩家者**留在佇列**（延後），不啟動、不佔用當前玩家。保留原本 at-most-once／「未知時代資料保留在佇列並回報錯誤」／「啟動成功才 pop、apply 效果縱使拋錯佇列也已 pop」等語義。
    - `_end_turn()`（`server/game.py`）在推進 `current_player_index` 之後、`turn_phase=ACTION` 之前，補一次 `_continue_era_activation_queue()`：讓「延後的時代」在其對象成為當前玩家的那一刻才啟動成該對象**自己**的可解 choice（wrap 與非 wrap 路徑都涵蓋；wrap 路徑與既有 5508 的 drain 冪等，遇 pending_choice 提早返回）。
  - 為何「雙方都卡」與修法後為何不卡：舊行為讓非紅軍當前玩家被紅軍的 pending_choice 全域擋死、紅軍又非 active＝雙方卡；修法後 round-wrap 當下該時代**延後**（佇列留著、無 pending_choice），非紅軍當前玩家**完全不被卡**、可正常打完自己回合；輪到紅軍時該時代才啟動成紅軍自己回合的 choice，紅軍解完後『上海合作組織』的 `scoped_card_range` 才真正套用（事件仍序列在時代之後）。時代「偵測」仍在整輪 wrap（未改），只改互動「啟動」的時機。
  - 新增回歸：`scripts/tests/test_action_card_regressions.py::test_shanghai_cooperation_org_at_round_wrap_does_not_deadlock_non_red_actor`（2 人局：非紅軍藏國起始＋紅軍；強制事件抽『上海合作組織』；驅動 round-wrap 後斷言非紅軍當前玩家**未被卡**（`pending_choice is None`、`advance_turn_phase()` 成功、時代留在 `_pending_era_activations`、事件 `auto_deferred`），續打到紅軍回合時時代才啟動成紅軍自己的 choice，紅軍解完後『上海合作組織』`scoped_card_range` 確實套用、事件 settled）。並改寫兩支原本把「round-wrap 當下就對紅軍啟動互動時代、由紅軍越回合解」當預期的既有測試 `scripts/tests/test_era_lifecycle.py`：`test_tibet_activation_choice_completes_before_same_boundary_auto_event_choice`→`test_tibet_red_suppression_defers_to_red_turn_instead_of_blocking_non_red_actor`、`test_simultaneous_tibet_and_manchuria_eras_serialize_before_auto_event`（改為驗證兩個互動時代各自延後到其對象（滿洲、紅軍）自己回合才啟動、皆在事件之前解完；保留「時代序列在 auto 事件之前」的實質不變量，只更正過時的時機預期）。`test_manchuria_activation_choice_completes_before_same_boundary_auto_event_choice` 因滿洲本身即起始/當前玩家而無需更動。
  - 驗證：完整 pytest（依指定 ignore 清單）**修改前 268 passed / 0 failed → 修改後 269 passed / 0 failed**（+1 為新增回歸）。`test_era_lifecycle.py` 11/11。era／勝利／時機 validators 全綠：`validate_era_effects_runtime` 15/15、`validate_era_rules` 5/5、`validate_era_canonical_scope` 8/8、`validate_event_outcome_timing_audit` 28/28、`validate_red_faction_inspect_reorder` 5/5、`validate_turn20_victory_declaration` 3/3、`validate_victory_rules` 5/5。（`validate_event_cards_runtime` 的 `event_red_dissolve` app.js 斷言為**既有失敗**，在未修改的 main repo 同樣失敗，與本次後端改動無關。）勝利判定時機完全未動。

### P2：雙方都持有情報網／爆料黑幕時，被取消的一方應該也能反過來取消對方的『取消』
- [done] 使用者回報：當雙方同時各自有『情報網』和『爆料黑幕』，任一方行動的時候，被另一方取消行動，前者應該也要可以取消對方的『取消』。（2026-08-05 使用者回報，先記錄，尚未調查）
  - **2026-08-05 調查（尚未實作，刻意與上面 P2『第二次爆料黑幕』分開交付，避免同一會話一次塞太多高風險改動到這個已多次翻修的反應子系統）**：確認目前**完全沒有**這個機制。反應者選擇取消時（`_resolve_reaction_choice()` 的 `card_choice_index` 分支，`server/game.py:4700` 起）只 `_build_reaction_context()`（`server/game.py:4747`，把反應卡從手上 pop、記 log、依被取消牌的費用設 `canceled_propaganda_card`/`canceled_money_cost_card` bonus flag）→ `_resume_reaction_pending_action(reaction_context=...)` → `_resolve_reaction_context()`（`server/game.py:4805`，執行反應卡效果並棄掉）。**沒有任何一步**把「打出反應卡」本身當成一個新的、可被再取消的『出牌事件』遞迴回 `_reaction_prompt_candidates`。現有的 `remaining_candidates`（`_set_pending_reaction_choice`）只是**平面鏈**（候選1放棄→問候選2→…，都在對『同一張原始牌』反應），不是遞迴鏈。
  - **關鍵可行性發現**：`cancel_card` effect（`server/effect_engine.py:604`）其實只是記一行 log——真正的「取消」是由呼叫端**不執行**被取消的牌達成的；反應卡（爆料黑幕／產業滲透）唯一的額外副作用是 `conditional_draw`（依**它直接取消的那張牌**是否含宣傳/資金費用而多抽1）。因此遞迴語意乾淨：這是一個**交替堆疊**——stack[0]=原始牌、stack[1]=B 的爆料黑幕(target stack[0])、stack[2]=A 的反制(target stack[1])…；由頂端往下 `resolved[top]=True`、`resolved[k]=not resolved[k+1]`，故原始牌結算 ⇔ 其上的反應卡數為**偶數**。每張反應卡的 bonus 抽牌只在它自己 `resolved` 時給、且看它 target 的那張牌費用。
  - **建議實作方向（下次接手）**：把「B 打出反應卡」導回一次 `_reaction_prompt_candidates(acting_player=B, played_card=<B 的反應卡>, ...)`（用反應卡自己的印刷費用算候選、排除 B）。若有候選就在 `pending_choice` 掛一個 layer stack（每層存 acting_player／played_card／cost／被選反應卡），開新反應視窗；沒有才走現行 `_resolve_reaction_context`。最終結算時依上述交替規則決定哪些效果實際發生、棄掉所有已花掉的反應卡、給該給的 bonus 抽牌。**本次 P2『第二次爆料黑幕』的延後結算重構已經把地基鋪好**——`play_card`/`_resume_reaction_pending_action` 現在都以「先開反應視窗、再結算效果」為統一模型，遞迴層可掛在同一套機制上。
  - **未決的產品設計（需使用者拍板，先別亂猜就實作）**：遞迴層與既有平面 `remaining_candidates` 鏈**如何組合**是真正的設計選擇，非機械延伸。情境：A 出牌，B 與 C 都持可取消卡→（平面）先問 B→B 取消→（遞迴）A 反制→**接下來該問誰**？(a) 只在 A↔B 兩造之間交替？(b) A 反制後，原本還沒被問到的 C 要不要也能對這條新鏈反應？(c) C 只能反應最初那張原始牌、還是也能插進反制鏈？這幾種都自洽但體驗差很多。建議預設採「最防禦性、對稱」的解讀並在實作時明確標注，但因為 underspecified，接手前最好先向使用者確認一句。目前無任何實作、無測試；不要在沒定案前硬做。
  - **2026-08-05 產品設計拍板**：使用者確認採 (b)——A 反制後，C（以及任何尚未在這條鏈中出過牌的人，包含更早已放棄過的人）都能繼續對 A 的反制反應；不特殊化「原始出牌者」為特權角色，每一層都是同一套機制。
  - **2026-08-05 實作**：把「遞迴反制鏈」直接疊在既有的平面 `remaining_candidates` 機制之上，兩個維度正交——平面鏈處理「同一層裡還有誰沒被問過」，遞迴堆疊處理「有人取消後開新的一層」。
    - **資料結構**：`pending_choice['resolution_stack']`（`server/game.py:4682` `_set_pending_reaction_choice` 新增的 `resolution_stack` 參數）是由下往上的 frame 陣列——`stack[0]` 是 `{'kind':'original','choice':{...}}`（原本 `_resolve_reaction_choice` 內建構、傳給 `_resume_reaction_pending_action` 的那個 `choice` dict，含 `acting_player`／`played_card`／`effective_type`／`action_context`／`support_resolution`），`stack[k>=1]` 是 `{'kind':'reaction','reaction_context':{...}}`（`_build_reaction_context()` 回傳的既有 dict，含 `reacting_player`／`reaction_card`／`reaction_card_name`／`canceled_card_name`／`canceled_card_cost`，無需新欄位）。第一層由 `_set_pending_reaction_choice` 在 `resolution_stack=None` 時自動建立 `[stack[0]]`；每多一次「取消」就在 `_resolve_reaction_choice`（`server/game.py:4728`）的 `card_choice_index` 分支 push 一個新的 `reaction` frame，並帶著累積的 `resolution_stack` 往下傳。單純「放棄」（`index==0`）不會 push 新 frame，只會沿用既有的平面 `remaining_candidates` 問下一位候選人——完全比對「新層只在真的有人取消時開，而不是有人放棄時開」的規格。
    - **開新層的觸發點**：`_resolve_reaction_choice` 的取消分支（`server/game.py:4728`）不再像舊版那樣直接呼叫 `_resume_reaction_pending_action(reaction_context=...)` 結算；而是把「B 打出反應卡」本身當成一次新的出牌，呼叫 `self._reaction_prompt_candidates(player, reaction_card)`（`player`=剛出牌的反應者，`reaction_card`=B 剛打出的那張卡，用它自己的印刷費用重新算候選、天然排除 B 本人）。若有候選，開新的 `reaction_choice` pending_choice（`opened_counter_layer: True`）；若沒有候選（沒人能反制，或唯一持卡人已在鏈中用掉），呼叫新方法 `_finalize_reaction_stack()` 一次結算整條鏈。
    - **結算演算法**（`_finalize_reaction_stack`，`server/game.py:4838`）：設 `n = len(stack) - 1`（反應卡張數）。`original_resolves = (n % 2 == 0)`。由上而下走訪 `k = n, n-1, …, 1`：若 `(n-k)` 為偶數（該 frame 上方存活的取消次數為偶數，即它離「頂端」的距離是偶數步）→ 這張反應卡**生效**：先 `_apply_reaction_cancel_flags(ctx)`（`server/game.py:4810`，依它直接取消的那張牌的印刷費用重設 `canceled_propaganda_card`/`canceled_money_cost_card`，因為多層鏈中每個 frame target 的牌不同，`turn_log` 上的值不能沿用某一層建立時的舊值），再 `_resolve_reaction_context(ctx)`（既有函式，執行該反應卡自己的效果＋棄牌/歸還）；否則該反應卡**被取消、不生效**，只消耗掉（`_discard_consumed_reaction_card()`，`server/game.py:4824`，純粹棄牌/topdeck，不跑效果、不給 bonus 抽牌）。`k==1`（直接取消原始牌的那張反應卡）在迴圈中特殊處理：若 `original_resolves` 為 False（frame1 本身存活，原始牌被取消），迴圈跳過它，交由既有的 `_resume_reaction_pending_action(orig_choice, reaction_context=stack[1], skip_reaction_resolution=False)`（`server/game.py:4563`，新增 `skip_reaction_resolution` 參數）走它原本「原始牌被取消」的既有分支（不執行原始效果＋解析 frame1 的反應效果），避免與迴圈重複結算同一個 frame。若 `original_resolves` 為 True，迴圈正常結算 `k=1`（此時 frame1 被判定為不生效，只消耗），迴圈結束後呼叫 `_resume_reaction_pending_action(orig_choice, reaction_context=None)` 執行原始牌的真正效果，等同它從未被取消過。
    - **手算驗證（實作前先手推，覆蓋深度1～4）**：
      - 深度1（`n=0`，無人取消）：`original_resolves=True`（`0%2==0`）；迴圈範圍空；直接執行原始牌效果。=既有行為。
      - 深度2（`n=1`，B 取消、無反制）：`original_resolves=False`；迴圈 `k=1` 因 `not original_resolves and k==1` 被跳過；`_resume_reaction_pending_action` 用 `reaction_context=stack[1]` 走「原始牌被取消」分支：原始牌不執行、B 的反應卡效果照跑。=既有行為（回歸）。
      - 深度3（B 取消、A 反制，無人再反制）：`n=2`，`original_resolves=True`；迴圈 `k=2`：`(n-k)=0` 偶數→A 的反制卡生效（含 bonus 抽牌）；`k=1`：`(n-k)=1` 奇數→B 的反應卡只消耗、不生效；迴圈後執行原始牌真正效果。=A 的原始牌如同從未被取消。
      - 深度4（B 取消、A 反制、C 再反制 A 的反制）：`n=3`，`original_resolves=False`；迴圈 `k=3`：`(n-k)=0` 偶數→C 的反制卡生效（含 bonus 抽牌）；`k=2`：`(n-k)=1` 奇數→A 的反制卡只消耗、不生效；`k=1` 因 `not original_resolves and k==1` 跳過，交給 `_resume_reaction_pending_action`：原始牌不執行，B 的反應卡效果**照跑**（翻回生效）。=A 的原始牌最終仍被取消，且是被「翻回生效」的 B 的反應卡取消，不是被 C 直接取消。
    - **`情報網` 用作反應卡的既有特例維持不變**：`_resolve_reaction_context()`（`server/game.py:4944`）對 `reaction_card_name == '情報網'` 固定只執行 `effect_engine.execute({'type':'cancel_card'}, ...)`（純 log），完全不跑它印刷的 `choose_one`（放內鬥／瓦解）——這是 2026-08-02 就有的既有規則（情報網當反應卡用時卡面選單不適用），本次遞迴堆疊沿用同一個函式，行為不變、無需特殊處理。
    - **既有 side-effect 追蹤語意刻意不變**：`_track_event_progress('play_card_with_money'/'play_card_with_propaganda', ...)`、`turn_log['played_money_card'/'played_propaganda_card']` 這些「打出某張牌」的追蹤，原本就只在 `play_card()`（`server/game.py:4852`）對**最初那張牌**觸發一次，從未對後續反應卡的打出觸發過（`_build_reaction_context`／`_resolve_reaction_context` 從未呼叫這些追蹤）。本次遞迴實作沒有新增任何呼叫，所以「反應卡打出本身」依然**不**計入這些觸發——與取消前的既有行為一致，沒有引入新語意。
    - **不需要特殊化「原始出牌者」**：`_reaction_prompt_candidates` 每次都用「排除剛出牌的那個人」重新計算，A、B、C 在遞迴中的地位完全對稱；某人在較早一層放棄過，只要手上還有其他合格卡，仍會在新一層被重新問到（`_resolve_reaction_choice` 的取消分支永遠重新呼叫 `_reaction_prompt_candidates`，不沿用舊層的候選名單）。終止性由「每張反應卡一旦打出即從手上 `pop`（`_build_reaction_context`，`server/game.py:4886`）」天然保證，不需要額外的「已詢問名單」或迴圈保護。
    - **前端／UI**：完全不需要修改 `static/app.js`。`Game.state()`（`server/game.py:6556` 起）序列化 `pending_choice` 時只曝光 `type`/`choice_key`/`cards`/`prompt`/`acting_player_name`/`played_card_name` 等既有欄位（`server/game.py:6653-6699`），**不**包含新增的 `resolution_stack`（純後端內部狀態）；每一層新開的 `reaction_choice` 對前端來說都只是「又一個一般的 reaction_choice pending_choice」，用的是同一顆 `acting_player_name`/`played_card_name`/`cards` 欄位（每層由 `_set_pending_reaction_choice` 依當層的出牌者/出牌重新填值）。既有的 `choiceType === 'reaction_choice'` 渲染（`static/app.js:1940` 起，「不取消」＋每張候選卡各一個按鈕）與 10 秒逾時自動放棄（`server/main.py:37` `schedule_reaction_timeout`，掛在每次 WebSocket action 之後、依當前 `game.pending_choice` 重新排程，`server/main.py:682`）對任何深度的層都原樣適用，未做任何修改也未發現需要修改之處——**多層鏈是後端與前端都完整支援，不是只有後端**。
  - **改動檔案**：`server/game.py`——`_resume_reaction_pending_action`（4563，新增 `skip_reaction_resolution` 參數，3處內部 `_resolve_reaction_context` 呼叫改為條件式）、`_set_pending_reaction_choice`（4682，新增 `resolution_stack` 參數與首層自動建構）、`_resolve_reaction_choice`（4728，取消分支改為 push frame＋開新層或呼叫 `_finalize_reaction_stack`）、新增 `_apply_reaction_cancel_flags`（4810）、`_discard_consumed_reaction_card`（4824）、`_finalize_reaction_stack`（4838）。`scripts/tests/test_action_card_regressions.py`——新增4個測試＋修正1個既有測試的尾段斷言（見下）。
  - **測試**（`scripts/tests/test_action_card_regressions.py`）：
    - `test_counter_cancel_depth3_lets_original_action_resolve`（1062）：A 打 領導、B 用 情報網 取消、A 用 爆料黑幕 反制——驗證 A 的 領導 效果正常執行（抽牌），B 的 情報網 完全不生效，A 的 爆料黑幕 因成功取消含宣傳費用的 情報網 而拿到 bonus 抽牌。
    - `test_counter_cancel_depth3_with_bystander_who_always_declines`（1108）：三人局，C 全程持有合格反應卡但每層都選擇不取消，結果與只有 A、B 兩人時完全一致（只多了詢問，沒有改變結算）。
    - `test_counter_cancel_depth4_third_player_joins_chain_and_recancels_original`（1154）：使用者確認的產品設計場景——A 打牌、B 取消、A 反制、C 再反制 A 的反制；驗證交替結算翻回原點，A 的原始牌最終仍被取消（且是被 B「翻回生效」的反應卡取消，符合手算深度4）。
    - `test_counter_cancel_chain_cannot_ask_same_reactor_twice_with_one_card`（1211）：終止性檢查——B 只有一張合格反應卡，用掉之後鏈條繼續（A 反制），B 不會被同一條鏈再問一次（因為手牌已空、`_reaction_prompt_candidates` 天然排除它），鏈條在此自然結束而非人為截斷。
    - `test_reaction_candidate_who_declines_lets_the_next_eligible_reactor_react_to_the_same_card`（1009，既有測試，尾段補寫）：既有的「平面鏈＋單次取消」測試，補上第二段斷言——second_reactor 用 爆料黑幕 取消後，那張 爆料黑幕 本身也開了一層新的反制視窗問 first_reactor（其手上還有 產業滲透），驗證**平面候選鏈與遞迴反制鏈可以正確組合**；此測試讓 first_reactor 選擇不反制，驗證鏈條在深度2收斂，與加入反制鏈前的最終結果一致（回歸保證）。
  - **驗證**：新增/改動測試單獨跑 4 passed（`test_counter_cancel_depth3_lets_original_action_resolve`／`_with_bystander_who_always_declines`／`_depth4_third_player_joins_chain_and_recancels_original`／`_chain_cannot_ask_same_reactor_twice_with_one_card`）＋既有反應相關測試 31 passed（含改動過的 `test_reaction_candidate_who_declines_...`）。完整可收集 pytest（`--ignore` 掉 4 個 WS/Playwright/Lobby 檔）：實作前 **264 passed**，實作＋補測後 **268 passed, 0 failed**（264 基準 + 4 個新增深度測試；既有測試數量不變，只有1個既有測試的斷言被更新以反映新增的反制層行為，不是刪測試）。另外重跑 `scripts/validate_support_no_reaction_phase_gating.py`（2/2 passed）、`scripts/validate_cancellable_choice.py`（7/7 passed）、`scripts/validate_intel_network_no_reaction_option_own_turn.py`（2/2 passed），皆無回歸。已知且與本次改動無關的 `scripts/validate_cost_composition_triggers.py::support_card_still_excluded_from_cost_triggers` pre-existing failure維持原樣、未處理（不在本次範圍）。
...
- [done] 使用者回報：先使用『宣傳家』，此時對方有跳出『爆料黑幕』的通知；接著再使用『東洋奧援』，對方這次卻沒有跳出『爆料黑幕』的通知。（2026-08-05 使用者回報）
  - **根因**：`_reaction_prompt_candidates()`（`server/game.py:4488`）開頭有一段刻意的 workaround——`if getattr(played_card, 'card_type', None) == 'support': return []`，直接讓**所有奧援卡**永不觸發取消反應。宣傳家是 command 卡、不受此排除，所以第一次正常跳；東洋奧援是 support 卡（`effective_card_type` 映射 `'東洋奧援': 'support'`），被無條件排除，這就是「第二次完全不跳」的全部成因。這段 workaround 的註解說明了它為何存在：奧援卡效果在 `play_card()` 內**即時結算**（support 分支 `support_resolution = self._execute_support_card(...)`），等效果都已改動盤面／開好互動選擇後才輪到檢查反應候選，此時再跳取消詢問會留下 stale `pending_choice` 卡住購買階段（正是 2026-07-04 那次 `validate_support_no_reaction_phase_gating.py` 要擋的問題）。對照 command 卡路徑：`action_engine.execute()` **不**在 `play_card()` 內即時呼叫，而是延後到反應視窗結算後的 `_resume_reaction_pending_action()` 才執行——這個「先問反應、再結算效果」的延後模式才是正解，奧援卡缺的就是這一套。
  - **修法（把 support 卡改走 command 卡那套「延後結算」）**：
    - 新增 `_support_interaction_targets()`（`server/game.py:2835`）作為互動型奧援「有無合法目標／目標清單」的**單一真相來源**；`_start_support_interaction()`（`server/game.py:2866`）改為呼叫它（同一份 prompt/kwargs、行為不變，只是把目標計算集中），新增的非變動預檢 `_support_card_has_legal_target()`（`server/game.py:4515`）也共用它，兩者永不會對「這張出牌合不合法」給出不同答案。
    - `_reaction_prompt_candidates()` 加 `include_support=False` 參數：預設仍維持 support 卡不出候選（保護紅軍奧援等既有路徑），只有新的延後 support 路徑傳 `include_support=True` 取得真正候選。
    - `play_card()` 的 support 分支（`server/game.py:5004`）重寫：先用 `_support_card_has_legal_target()` **非變動預檢**，無合法目標就地退回手牌／還原 cost flag（沿用舊 no_legal_target 語意，但改在**動盤面之前**判定，避免白白花掉對手一張反應卡去取消一個不合法的出牌）；接著照舊即時記 `play_card_with_money/propaganda` 事件進度與 `played_nonstarter_names`（出牌即計，就算之後被取消也計，與 command 卡一致）；然後**在結算前**開反應視窗——有候選就 `_set_pending_reaction_choice(..., support_resolution=None)` 直接 return，把 `_execute_support_card()` 延後；無候選才呼叫新的 `_commit_support_card_play()`（`server/game.py:4531`，把原本 4926-4962 那段「執行奧援卡＋處理其 pending_choice／北國奧援可回退」抽出來共用）。
    - `_resume_reaction_pending_action()`（`server/game.py:4563`）補上 support 分支（`server/game.py:4586`）：被取消（`card_canceled`）→ 只結算反應卡本身並棄掉奧援卡、**不**執行其盤面效果；未取消 → 呼叫 `_commit_support_card_play()` 真正結算，若開了互動選擇就停在該 pending、否則 fall-through 共用尾段（陣營能力觸發＋棄牌），與即時無反應路徑語意一致。
    - 涵蓋範圍：一般 support 分支的 8 張奧援卡（印度／英美／歐洲／南洋／東洋／北國／臺灣／天方奧援）全部可安全延後——互動型（建組織／瓦解／強制棄牌）本來就只是「開 pending choice、實際動盤面在選點結算時才發生」，延後天經地義；非互動型（gain_resource／draw／draw_then_discard／add_internal_conflict）延後只是把自我封閉的效果往後挪、無序列相依。**紅軍奧援不在此範圍**：它在 `play_card()` 前段（`server/game.py:4864` 起）就被特判抽牌＋傳牌處理掉，根本走不到 support 分支，維持現行「不跳反應」行為（如需比照另立項目）。
  - **順帶反轉一項舊決策（已於同批更新對應驗證）**：2026-07-04「奧援卡不再觸發取消反應」（TODO 下方 P1 UI 批次該項、及 `validate_support_no_reaction_phase_gating.py`）的**使用者可見行為**被本次直接反轉——奧援卡現在會跳取消詢問。但當年那個 bug（stale pending 卡住購買）的根因已用「反應視窗開在結算之前」正解掉，故 phase-gating 保證仍在。`validate_support_no_reaction_phase_gating.py` 已改寫：兩支測試分別驗「南洋奧援＋對手持爆料黑幕→跳反應→不取消→效果結算→仍能推進到購買/END 階段（無 stale pending）」與「取消→效果不發生→仍能推進階段」。
  - **新增回歸**（`scripts/tests/test_action_card_regressions.py`）：`test_support_card_now_prompts_cancel_reaction_and_cancel_prevents_build`（東洋奧援 III 級 interactive build，取消後盤面零組織）、`test_support_card_cancel_reaction_declined_lets_the_build_resolve`（不取消→建組織互動照常開啟並完成）、`test_noninteractive_support_cancel_reaction_prevents_effect`（歐洲奧援獲宣傳，取消後宣傳=0）、`test_command_then_support_card_each_prompt_cancel_reaction`（**使用者原始情境**：宣傳家先跳、東洋奧援接著也跳）、`test_illegal_support_card_is_rejected_before_cancel_reaction_window`（無合法目標在開反應視窗前即退回、對手反應卡不被消耗）。另更新兩支既有測試以對齊新流程：`test_play_card_rolls_back_interactive_support_when_no_legal_target` 改 monkeypatch 共用的 `_support_interaction_targets`（而非下游 `_start_support_interaction`）、`test_tianfang_support_with_no_in_range_target_does_not_silently_discard` 改斷言 play_card 自己的 `could not play …: no legal target` log。
  - **驗證**：完整 pytest（同 baseline ignore 清單）**259→264 passed, 0 failed**（恰為新增 5 支）。`validate_support_no_reaction_phase_gating.py` 2/2、`validate_cancellable_choice.py` 7/7、`validate_intel_network_no_reaction_option_own_turn.py` 2/2 全綠（確認北國奧援可回退、中紀委/政工部/國安部可取消 pending、情報網己方回合不誤觸、情報網反應路徑仍正確取消等既有行為未被波及）。`validate_cost_composition_triggers.py` 的 `support_card_still_excluded_from_cost_triggers` 為**先前既有失敗**（git stash 回 base commit 亦 8/9 fail），與本次無關、未觸碰。
...
- [done] 使用者提出：宣布勝利也應該要等這一輪所有玩家（包含紅軍）都行動完才公布，因為紅軍有可能在同一輪稍後行動，把其他玩家的組織瓦解掉，改變原本的勝負結果。（2026-08-03 使用者提出）
  ...
  - **2026-08-04 實作（採用與上方時代觸發相同的整輪 wrap 邊界機制；此為該修正的孿生項）**：
    - 根因：`_end_turn()`（`server/game.py:5190`）在函式最開頭無條件呼叫 `_check_victory()`——也就是每位玩家自己回合結束（`current_player_index` 尚未推進之前）就判定一次。只要某位非紅玩家自己回合結束當下快照符合勝利條件，就立刻 `game_phase=FINISHED`、`return`，中止整輪剩餘玩家（含紅軍）的行動；即使紅軍當輪稍後才行動、可能把撐住條件的組織瓦解掉，也已誤判宣告。整輪 wrap 區塊（`if self.current_player_index == round_start_player_index:`，`server/game.py:5240`）本來就另有一個 `_check_victory()`（第20回合保底判定用），所以 wrap 回合其實會判定兩次、非 wrap 回合判定一次。
    - 修法：比時代觸發單純得多——`_check_victory()`／`victory_engine.evaluate()`（`server/victory.py:9`）是純快照評估，沒有互動式 pending choice 佇列、沒有跨回合續跑狀態、沒有任何副作用（唯一副作用是達標時設 `FINISHED`／`winner`／`co_winners` 並結束遊戲），因此**不需要**像時代那樣拆「偵測 vs 續跑」，只要把逐回合的判定點拿掉即可。直接移除 `_end_turn()` 開頭那個無條件 `_check_victory()` 呼叫，只保留整輪 wrap 區塊內原本就有的那一個（`server/game.py:5245`，該點是整輪最後一位玩家回合結束、`current_player_index` 剛好繞回 `round_start_player_index` 的當下，代表含紅軍在內每位玩家本輪皆已行動）；開頭原本的 `if self.game_phase == FINISHED: return` 守衛保留為防禦性 no-op（避免遊戲已在前一次 wrap 結束後又被重入 `_end_turn`）。wrap 區塊的 `_check_victory()` 排在 `self.turn += 1` 之後，第20回合保底（`turn > 20` → 紅軍勝）時機完全不受影響。
    - 其他呼叫點稽核：全庫 `_check_victory(` 僅三處——上述兩處對局呼叫點，外加 `server/main.py:934`。後者位於 `/test/*` scope 稽核情境設定端點（`setup-scope-audit`），是為了測試 fixture 立即把狀態判定到位而強制檢查，與正常對局流程無關、屬合理例外（比照時代修正裡 `_check_era_trigger()` 保留給 `/test/*` 端點的處置），不動。
    - 新增回歸（`scripts/tests/test_victory_round_wrap.py`，比照 `test_era_lifecycle.py` 的整輪 wrap 三支）：`test_victory_deferred_to_round_wrap_so_red_army_can_still_invalidate_it`（非紅玩家自己回合已達牆內14勝利條件，但整輪 wrap 前紅軍瓦解掉1個組織使其掉回13，斷言非紅玩家自己回合結束不宣告、整輪 wrap 判定也因條件已破而不宣告）、`test_victory_declared_at_round_wrap_when_condition_survives_red_army_turn`（同條件在紅軍行動後仍成立，斷言非紅玩家自己回合中途不宣告、整輪 wrap 邊界才正確宣告 `winner`）、`test_turn20_red_army_fallback_still_declared_at_round_wrap`（確認第20回合保底判定仍在同一 wrap 呼叫點正確運作、turn 推進到21當下宣告紅軍勝）。修正 `scripts/validate_turn20_victory_declaration.py` 內把逐回合舊時機當預期的 `case_mid_round_player_win_unaffected`（原斷言非紅玩家自己回合結束立即宣告勝利）→ 改寫為 `case_mid_round_player_win_deferred_to_round_wrap`（斷言非 wrap 回合不宣告、紅軍行動後整輪 wrap 才宣告，只更正過時的時機預期並強化為同時涵蓋「延後」與「邊界宣告」兩面，未削弱覆蓋）。
    - 驗證：新增 3 支全綠；victory focused 全綠——`validate_victory_rules` 5/5、`validate_shared_victory_phase8` 3/3、`validate_turn20_victory_declaration` 3/3、`test_all_victory_scopes` 全過。完整 pytest 246 passed／2 failed（僅既有無關 FakePage API drift：`test_advance_to_action_waits_for_my_turn_and_advances_until_action_phase`、`test_wait_for_confirmed_faction_and_ready_state_use_latest_lobby_state`；baseline 243→246 恰為新增3支測試），其餘既有勝利／時代相關測試零回歸。時代觸發時機（上一項）本次不動。

### P1：事件卡『北京政爭』用網羅人才選到樹立信心應算任務成功；另網羅人才選完似乎沒有立即洗牌
- [done] 使用者playtest回報兩點：(1) 事件卡『北京政爭』，使用網羅人才、選到樹立信心，應該要算成功達成任務才對。(2) 同一回合內第二次使用網羅人才時，可選的候選卡牌只有一張——代表第一次使用網羅人才時，並沒有馬上直接把牌庫洗牌（卡面：「...而後將牌庫洗牌」）。（2026-08-03 使用者回報；2026-08-04 使用者要求開始處理）
  ...
  - **(1) 根因比原本想的更大**：稽核 `_track_event_progress()` 發現，`'draw'` 這個事件觸發類型目前**只有** `_draw_player_cards()`（`server/game.py:518`，紅軍奧援／各奧援卡／時代效果等呼叫的抽牌路徑）會呼叫 `_track_event_progress('draw', ...)`；一般透過 `effect_engine.py` 的 `draw`/`conditional_draw`/`shared_draw` 效果型別（也就是絕大多數一般行動卡的抽牌，例如樹立信心、領導、點燃熱情）都走 `effect_engine.py` 自己的 `_draw()`，完全沒有呼叫 `_track_event_progress`。也就是說：**不管是直接打出樹立信心，還是透過網羅人才把樹立信心加入手牌，都不會讓『北京政爭』這類 `trigger:{"type":"draw"}` 的事件任務推進**——網羅人才只是剛好也不算數的其中一種途徑，不是唯一漏掉的。全部門檢查過 `data/events_structured.v1.1.json`，只有『北京政爭』與2張『全國人大召開』（副本，trigger 是 `use_faction_ability`，不受影響）用到 `draw` 型別。
  - **修法**：`effect_engine.py` 的 `_draw()` 新增 `log` 參數（預設 `True`），抽到牌後只要有 `game` 就一律呼叫 `game._track_event_progress('draw', amount=len(drawn), player=player)`；原本的具名 log 訊息維持不變，只是改由 `log=False` 控制是否額外寫那行 log（`shared_draw` 效果型別的兩次內部 `_draw()` 呼叫改傳 `game=game, log=False`，讓兩位玩家的抽牌都計入任務進度，同時仍保留原本自己組合的合併 log 訊息，不會重複寫兩行）。`_event_trigger_actor_allowed()` 既有的紅軍排除邏輯不受影響——紅軍玩家自己的抽牌仍然不會讓任務進度前進。
  - 新增 `test_beijing_power_struggle_progresses_on_ordinary_draw_effect_card`（一般 `draw` 效果，如樹立信心）／`test_beijing_power_struggle_progresses_on_shared_draw_effect_card`（`shared_draw` 效果，如合作談判，雙方抽牌都計入）／`test_beijing_power_struggle_ignores_red_army_draws`（確認紅軍抽牌仍不算數，既有限制沒被放寬）（`scripts/tests/test_action_card_regressions.py`）。完整 pytest 255/255 全過（先前列在其他項目下的 2 個既有無關 FakePage API drift 失敗，已在上一輪順手修好 `scripts/tests/test_validate_ui_card_flows_runtime.py` 的假物件，目前全數清零）；另重跑 `validate_event_outcome_timing_audit.py`（28/28）確認事件結算時機相關邏輯無回歸。
...
- [done] 使用者playtest回報：先打出`宣傳家`（效果含 `build range:1` 與 `move count:1`）與`組織經驗丙`（效果含 `build range:1`），到地圖頁面後先在桃園建立第一個組織，接著移動到彰化，再要建立第二個組織時，預期候選城鎮應該考慮彰化附近（因為已經移動過去），但實際看到的候選仍是臺北附近的城鎮。（2026-08-03 使用者回報，先記錄，尚未深入稽核／未重現）
  ...
  - **2026-08-04 根因確認並修正**：用真正的 `play_card()`/`resolve_pending_choice()`/`move_organization()`（不是任何測試捷徑）重現「建立→移動→建立」序列，直接證實候選清單確實過期——`_activate_next_queued_card_build()` 只在佇列批次啟動當下（也就是上一次建立剛解決完那一刻）呼叫 `_card_build_town_choices()` 算一次，結果存進 `choice['towns']` 後就不再更新，直到玩家真正點擊解決那一刻，`_resolve_town_choice()` 也只是讀取這份存好的舊清單，完全沒有重新投影。`era_red_build_near_target`（時代紅軍加成建立）用不同函式 `_era_build_towns_near_target()`，但架構是同一類問題（同樣只在批次交接當下算一次）。
  - 修法：新增 `_live_pending_town_choices(player, choice)`（`server/game.py`，緊接在 `_resolve_town_choice()` 前面），對 `card_build_organization` 用 `_card_build_town_choices(player, choice['context']['effect'])`、對 `era_red_build_near_target` 用 `_era_build_towns_near_target()`（排除 `context['built_towns']`）即時重新投影，不信任 `choice['towns']` 裡存的舊清單。兩個呼叫點：(1) `state()` 序列化 pending_choice 給前端之前，用該 pending choice 的擁有者（不是目前呼叫 `state()` 的 viewer）重新投影一次，並把結果寫回 `self.pending_choice['towns']`；(2) `_resolve_town_choice()` 一開始就先重新投影再解析玩家送來的 index，確保伺服器端驗證與畫面呈現一致、都不會用到過期候選。
  - 新增 `test_card_build_organization_candidates_refresh_live_after_a_move_between_builds`（`scripts/tests/test_action_card_regressions.py`）：疊加組織經驗丙＋組織經驗乙進入3個建立額度的連續 session，在臺北先建立桃園，用真正的 `move_organization()` 把組織從桃園移到新竹，斷言：`state()` 回傳的候選清單正確反映新竹鄰近城鎮（苗栗）、已淨空的桃園重新變回合法候選、已經不再鄰接任何己方組織的宜蘭正確消失；並確認第二次建立仍可正常解決、額度正確遞減。`test_action_card_regressions.py` 全數109項通過；完整 pytest 240/242（2個既有無關失敗）；重跑 `validate_build_entitlement_queue_browser.py`（10/10）、`validate_org_exp_a_repeat_build.py`（5/5）、`validate_belt_road_red_turn_build_gating.py`、`validate_build_queue_preserves_map_viewport.py`（5/5）、`validate_buildable_town_count.py`（4/4）、`validate_safehouse_build_range.py`、`test_era_lifecycle.py`（8/8）、`validate_era_effects_runtime.py`（15/15）皆綠燈，確認沒有波及既有連續建立／時代紅軍加成建立流程。
...
- [done] 玩家累積多張可建立組織的卡牌並在戰略地圖連續建立時，每成功建立一個組織後應暫時保留使用者當下的Leaflet `center`與`zoom`，只更新組織marker、合法候選與剩餘建立數，不要重新 `fitBounds`、reset view或zoom out；因為下一個目標很可能就在相鄰城鎮。後續需區分「首次進入建立流程可自動定位」與「同一pending build queue內後續重繪必須保留viewport」，並涵蓋候選重算、相鄰／遠距下一目標、最後一次建立、iframe重新render、桌面／行動版及正式Leaflet UI proof。（2026-08-02 playtest建議）
  ...
  - 修法：`supportChoiceHighlightKey()` 只保留 `mode`／`actionKind`／`choiceKey` 三個在整個連續 session 內保證不變的欄位；`towns`／`prompt`／`sourceName`／`focusTown` 全部排除。同一 session 內後續每次重繪只更新 marker／候選／剩餘數，鏡頭原地不動。2026-08-22 再依新 playtest 規則擴充：session 結束後雖仍會清空目前 focus key，但同一遊戲／玩家地圖會記住已完成過首次建立聚焦；下一個獨立建立 session 不再重新 `fitBounds`，只保留玩家現有視角。切換遊戲或玩家時才重設首次聚焦狀態。此 key 同時驅動瓦解流程的地圖高亮；瓦解流程維持各自既有聚焦規則。
...
- [done] S5-1：香港根據地免費遷移規則（2026-08-14 依使用者最新裁決修正）。
  - 正式規則：事件卡**香港抗暴之戰**完成結算後（成功或失敗皆算），香港可在下一回合開始前免費遷移一次根據地至臺北／倫敦／卡加利／多倫多，亦可留在目前根據地。
  - 赤鱲角機場是另一項「花費 2 次遷移移動香港組織」的規則，不提供付費遷移根據地。舊紀錄把兩者混在一起，現已移除錯誤路徑。
  - 實作：事件結算開啟 `hk_free_base_relocation`；`relocate_hong_kong_base` 只接受免費窗口；新增 `keep_hong_kong_base`；根據地錨定組織與在地能力隨遷移切換；目的地已有組織時封鎖。
  - UI：大廳及「我的陣營」列出全部五個根據地與能力；窗口開啟時顯示四個遷移按鈕與「留在」按鈕。
  - 驗證：`validate_hk_base_relocation.py` 5/5、Browser proof 8/8、pytest 聚焦 8/8。
