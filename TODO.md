...
- [done] 更正playtest重現：目前觀察到的是 `產業滲透`第一次成功取消對手的卡牌能力後，到了下一回合，對手再次使用卡牌能力時似乎不會再出現取消詢問；尚不能據此斷定同一回合內只有第一次可取消。後續需先依canonical卡面確認效果持續期限，再稽核首次取消時是否錯誤永久消耗／移除了reaction entitlement，而不是只結束當次反應；建立跨回合「首次取消→下一回合再次出牌」為主的regression，並另外確認當次選擇不取消、同回合連續出牌、資源模式、不同玩家、效果正確到期與正式WebSocket UI反應流程。（2026-08-02 playtest更正）
  - 結論：稽核 `server/game.py` 的 `_reaction_prompt_candidates`／`_set_pending_reaction_choice`／`_end_turn` 後**未發現程式缺陷**。`reaction_prompted_player_ids` 旗標存在 `self.turn_log`，而 `turn_log` 只有唯一一個生產路徑會被整個換新——`_end_turn()`（`server/game.py:5155`，每次某玩家回合正式結束時呼叫）；不存在任何提早 return 或例外路徑會跳過這次換新，因此「同一回合內只問一次」的既有設計（`test_first_other_player_action_prompts_cancel_reaction_once_with_all_available_cards`）與「換到下一回合即重新允許詢問」兩者不衝突，且都已被驗證正確。
  - 根本原因（可解釋 playtest 觀察到的現象）：`產業滲透`／`爆料黑幕` 都是購買區抽卡（`copies: 4`，非固定手牌起始卡），成功取消後那一張實體卡會被棄置（`_resolve_reaction_context` 呼叫 `action_engine.execute` 後歸還／棄牌）。玩家若沒有第二張副本在手上，下一回合自然不會再跳出取消詢問——這是預期的「單次消耗型卡牌」行為，不是「反應資格被永久移除」的錯誤。
  - 新增 `scripts/tests/test_action_card_regressions.py` 三支跨回合 regression（透過真正的 `advance_turn_phase()`／`_end_turn()` 狀態機推進整輪回合，不手動竄改 `current_player_index`）：`test_產業滲透_reaction_prompt_reappears_on_a_later_turn_after_being_used`、`test_爆料黑幕_reaction_prompt_reappears_on_a_later_turn_after_being_used`（確認兩張卡共用的取消反應程式碼，行為一致）、`test_reaction_prompted_player_ids_only_suppresses_within_the_same_turn_not_forever`（直接斷言 `turn_log` 物件身分在 `_end_turn()` 後改變、`reaction_prompted_player_ids` 歸零，而非原地修改殘留）。三支皆給 reactor 補發第二張副本後驗證取消詢問確實在下一輪重新出現並可再次成功取消；順便發現並修正兩個測試撰寫瑕疵：(a) 選來觸發反應的卡（點燃熱情／領導）本身含資金／宣傳費用，會讓 `產業滲透`／`爆料黑幕` 自帶的 `conditional_draw` 加抽效果一併觸發，原本斷言手牌為空過於嚴格，已改為斷言「用掉的那張反應卡本身」離開手牌；(b) 整輪推進會在回合交接時抽到真實隨機事件，可能夾帶自己的 pending_choice，`_advance_one_full_turn` 輔助函式已改為推進後立即重新釘選無效果事件，而非推進前預先斷言 pending_choice 為空。全數 3/3 通過，完整 pytest 232/234（2 個既有無關 FakePage API drift 失敗）。
  - 未做：正式 WebSocket UI 層面的跨回合反應流程尚未另外建立雙玩家瀏覽器 proof（本次以直接呼叫 `play_card`／`resolve_pending_choice`／`advance_turn_phase` 等生產方法覆蓋核心狀態機，未經過 WS 序列化/前端渲染層；`reaction_choice` 的 WS 廣播與隱私範圍已有既有 `test_reaction_timeout_and_privacy.py` 覆蓋，但未疊加「跨回合」維度）。若之後仍在正式 UI 中重現「下一回合看不到詢問」，優先確認玩家手牌內是否仍有該張卡（而非懷疑此處邏輯），再視需要補建瀏覽器 proof。
