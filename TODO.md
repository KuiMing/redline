...
- [todo] 修正 structured event data 與 raw card text 不一致的事件。
  ...
  - raw 8 張時代關卡盤點：
    ...
    - 2026-05-27：已完成香港紅軍壓制 `bonus_discard_on_red_card` runtime：紅軍打出間諜類卡牌並完成原本間諜瓦解 pending choice 後，接續重用既有 card choice / multi-card choice 架構，要求香港玩家棄 1 張手牌；香港革命反撲武裝購買 -2 資金仍維持。`scripts/validate_era_effects_runtime.py` 擴充至 14 項 runtime checks；proof records：`docs/records/event-cards/ERA_EFFECTS_RUNTIME_VALIDATION.{md,json}`。
