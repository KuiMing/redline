...
### P1：`產業滲透`應對對手每次使用卡牌能力都重新詢問是否取消
- [todo] `產業滲透`生效期間，受影響的對手每次嘗試使用卡牌能力時，產業滲透的控制者原則上都應收到一次獨立的「是否取消」選擇；目前playtest看起來只有該回合第一次使用卡牌能力時會觸發取消機會。後續需稽核reaction entitlement是否被錯誤記成turn-level一次性旗標，確認選擇「不取消」或完成一次取消後都不會消耗後續卡牌的詢問權，並涵蓋同回合連續多張卡、同卡多段／pending效果、資源模式不誤觸發、不同玩家、跨回合重置與正式WebSocket UI反應流程。（2026-08-02 playtest回報）
