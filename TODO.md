...
- [todo] 臺灣時代關卡的「7個牆內組織」達成判定有誤：playtest中臺灣玩家只是在臺灣地區建立到8個組織，並沒有在牆內建立7個組織，畫面卻顯示條件已達成。`牆內`的authoritative定義應為「目前統治者／控制者是紅軍的城鎮」，不以地理區域名稱、組織擁有者陣營或臺灣玩家的組織總數判斷；位於這些紅軍統治城鎮中的任何陣營組織都應計入，位於非紅軍統治城鎮的組織則一律不計。後續需建立共用town-controller／inside-wall helper，依canonical城鎮統治資料動態判定，並涵蓋任意陣營組織、臺灣地區負例、統治者改變、恰好6／7、玩家可見進度與正式地圖／WebSocket時代達成proof。playtest截圖保存於 `docs/records/era-cards/taiwan-inside-wall-achievement-miscount.jpg`。（2026-08-02 playtest回報）
...
- [todo] 使用 `北國奧援`、`臺灣奧援`或其他需要選擇組織進行瓦解的卡牌／效果時，建立authoritative pending target後應自動切換至正式戰略地圖，並只在後端判定可合法瓦解的組織marker上直接顯示 `💀`；玩家應可在地圖上點選該組織完成瓦解，不必先在指揮中心或通用choice modal尋找目標。後續需統一各瓦解來源的 `interaction_kind`與候選投影，確認敵我、距離、地區、根據地保護、自我犧牲、無合法目標fail-closed及結算後返回正確視圖，並建立正式Leaflet／WebSocket UI proof。（2026-08-02 playtest建議）
