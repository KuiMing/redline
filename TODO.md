...
- [todo] 使用者回報：戰略地圖選「陣營」篩選為香港後按「聚焦結果」，畫面不會真的 zoom in
  到香港地區。使用者知道河內、胡志明市及臺灣三座城鎮也是香港陣營合法建立組織的城鎮
  （因此會被一併篩選出來、算進聚焦範圍），但聚焦後的縮放程度感覺還是不夠。
- [todo] 初步看程式碼（`static/leaflet_game_map_logic.js` 的 `fitVisible()`）：陣營篩選
  時已經改用「結果城鎮座標中位數」當聚焦中心（`medianNumber`），避免海外離群城鎮把畫面
  拉回全亞洲——但縮放層級 `targetZoom` 仍是用 `map.getBoundsZoom(bounds, ...)` 對「全部
  篩選結果」的原始 bounds（含河內/胡志明/臺灣等離群城鎮）計算出來的 `fittedZoom`，只是
  額外 `Math.min(9, Math.max(4, fittedZoom))` 夾在 4~9 之間；中心點雖然已經修正到主要
  群集，但縮放層級本身可能還是被離群城鎮拉低，這很可能是「中心點對了、但還是不夠近」
  的根因，需要之後改成只用主要群集（例如排除離群點後的子集，或直接以固定聚焦半徑取代
  bounds-derived zoom）重新計算 `fittedZoom`。尚未動手修，此為使用者要求的初步記錄。
