...
### P2：勝利摘要文字加大並改善資訊密度
- [ ] 回報情境：遊戲結束畫面的勝利摘要文字偏小；摘要面板下方留下較多空白，訊息空間看起來太空。
- [ ] 需求：適度放大勝利標題、回合資訊、勝利敘述及玩家統計文字，並調整內容配置，使摘要面板更充分使用現有空間。
- [ ] 限制：保留全畫面勝利背景、觀看最終盤面按鈕、玩家統計內容與 viewer-specific 勝利敘述；不可讓長敘述或多人統計裁切。
- [ ] 驗收：至少覆蓋短敘述、長敘述、兩位玩家與多人統計；驗證 1280×720 與 1024×768，並確認文字清楚、面板不溢出、背景主體仍可辨識。
- [ ] 需求圖：`docs/records/ui-layout/victory-summary-text-density/request_victory_summary_text_density_20260823.jpg`。
- [ ] 本次只記錄需求；尚未修改功能。
