"""Test-only route for the 紅軍權貴出逃 event proof state."""
...
class EliteDefectionEventTestRoutes:
    def __init__(self, runtime_provider: Callable[[], GameSetupRuntime]):
        self._runtime_provider = runtime_provider
        self.router = APIRouter()
        self.router.add_api_route(
            "/test/setup-elite-defection-event-proof",
            self.test_setup_elite_defection_event_proof,
            methods=["POST"],
        )

    def test_setup_elite_defection_event_proof(self, payload: dict):
