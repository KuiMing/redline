"""Test-only route for the expand-results proof state."""
...
class ExpandResultsTestRoutes:
    def __init__(self, runtime_provider: Callable[[], GameSetupRuntime]):
        self._runtime_provider = runtime_provider
        self.router = APIRouter()
        self.router.add_api_route(
            "/test/setup-expand-results-proof",
            self.test_setup_expand_results_proof,
            methods=["POST"],
        )

    def test_setup_expand_results_proof(self, payload: dict):
        ...
        game.id = game_id
        game.log(
            "UI proof setup: viewer has 擴大戰果; discard pile contains 宣傳家 / 合作談判 / 走漏風聲."
        )
