...
- `docs/records/map-ui/MAP_NO_RADIAL_LINES_20260709_065808.{json,md,png}` 與 `scripts/validate_map_no_radial_lines.py` 也已經 staged（新檔案），是對應上述修正的驗證產物。
...
3. 現有 `scripts/validate_map_no_radial_lines.py` 只用 `__selectTownForTest('臺北')` 測了單一情境（單一玩家、單一城鎮，沒有特別驗證同時有道路+鐵路多個可達目標、也沒有模擬「先建立組織再自動重新高亮」這種宣傳家報告情境）。請擴充同一支腳本（或在裡面新增第二個情境函式），至少涵蓋：
...
8. 撰寫新驗證腳本（例如 `scripts/validate_move_confirmation.py`，可參考 `scripts/validate_map_no_radial_lines.py` 的 Playwright 寫法），至少覆蓋：
...
5. 撰寫/擴充瀏覽器驗證（例如新增 `scripts/validate_lobby_single_copy_button.py`）：載入 lobby、建立或帶入一個房間代碼、斷言畫面上只剩一個可用的複製按鈕（`#copyRoomBtn`）、點擊後透過既有的 `window.__lastRoomCopyResult` 確認複製成功、並確認橫幅上原本的複製按鈕已經不存在。截圖存證放 `docs/records/`（可放 `docs/records/playtest-feedback/` 或新開一個 lobby-ui 目錄，統一放一處）。
