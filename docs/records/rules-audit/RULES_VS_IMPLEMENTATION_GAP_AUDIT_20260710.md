...
- ⏳ A2 剩餘：`民族祭儀` 與能力文字的三個偏差（放牌庫底手牌未讓玩家選擇、沒猜中未提供「2宣傳或2資金」二選一、展示的頂牌被棄掉需確認去向——`賭徒耳語` 亦有前後兩項同樣問題）。
...
| [盤點誤判更正＋部分修正 2026-07-11] 【民族祭儀】 | 12 個陣營：`dian_zhuang, zhuang, yi, bai, hani, dai, miao, tujia, dong, buyei, yao, li`（雲貴少數民族類陣營） | 「在己方行動階段，可將1張手牌放進牌庫底，猜牌庫頂牌購買費用奇偶，並展示牌庫頂牌。若猜中可獲得2點宣傳與2點資金，沒猜中則獲得2點宣傳或2點資金。」 | **原盤點「完全沒有實作」是誤判**：runtime 分支（與 `賭徒耳語` 共用的猜奇偶邏輯，含猜中2+2/沒猜中2宣傳的獎勵差異）與前端 UI（12 陣營按鈕、猜奇偶 modal、結果顯示）早已存在，缺的只是 `_resolve_ability_text` 名稱對照（2026-07-11 已補上，配合新的後端歸屬檢查）。**仍殘留三個與能力文字的偏差**：①放牌庫底的手牌是寫死 `hand.pop()`，未讓玩家選擇（`賭徒耳語` 同）；②沒猜中應「2點宣傳**或**2點資金」二選一，目前固定給宣傳；③展示的頂牌直接進棄牌堆，文字只說「展示」，去向需確認（`賭徒耳語` 同）——③屬第三梯隊待確認。 |
