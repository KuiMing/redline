...
- ✅ 已完成：**A2（部分）：`非暴力`**（民運派、性別革命）—— 2026-07-11 修正並驗證，詳見上方 A2 段落與 `TODO.md`「規則資料 vs 程式實作落差修正」條目；A2 剩下 `紅軍派系`、`民族祭儀` 仍待處理。
...
3. 「民族祭儀」建議直接參考 `賭徒耳語` 的猜奇偶程式碼結構改寫（效果數值不同：猜中拿宣傳+資金各2點、沒猜中二選一拿2點宣傳或2點資金，且成本是「放1張手牌到牌庫底」而非賭徒耳語原本的成本，需要核對清楚兩者是否共用同一張源卡或各自獨立）。**`紅軍派系`、`民族祭儀` 仍待處理。**
...
3. **A2（3 個能力字串未解析，影響 15 個陣營）**——~~「非暴力」（民運派、性別革命，影響2個陣營）~~ **已於 2026-07-11 修正**；剩下 `紅軍派系`（改革開放派，1個陣營）、`民族祭儀`（12個雲貴少數民族陣營）仍待處理，`民族祭儀`可參考 `賭徒耳語` 改寫。
