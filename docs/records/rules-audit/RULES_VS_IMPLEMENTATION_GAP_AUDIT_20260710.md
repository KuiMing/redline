...
> **2026-07-11 補充**：已完成第二輪通盤盤點，發現 7 個本報告未涵蓋的新落差（組織棋供應上限、新疆社會管控、事件牌庫張數、開局額外卡去向、香港特殊規則、無限行動規則、翻牆成本程式碼層級確認），並整理了「第一輪剩餘＋第二輪新發現」的合併優先順序，見 `docs/records/rules-audit/RULES_VS_IMPLEMENTATION_GAP_AUDIT_SECOND_PASS_20260711.md`。
...
- ✅ 已完成：**`情報網`**（非反應時機下選項C選了沒效果，B3）—— 2026-07-10 修正並驗證，詳見上方段落與 `TODO.md`「規則資料 vs 程式實作落差修正」條目。
- ✅ 已完成：**`樹立信心`／`點燃熱情`／4個陣營能力**（`商貿組織`、`基金會`/`共合會`、`民族調和`/`星星之火`、`人同此心` 條件判定看種類而非購買費用，B3）—— 2026-07-10 修正並驗證，詳見上方段落與 `TODO.md`「規則資料 vs 程式實作落差修正」條目。
- ✅ 已完成：**B1-b**（`南洋奧援` I 級棄牌等於沒抽沒棄）—— 2026-07-11 修正並驗證，詳見上方段落與 `TODO.md`「規則資料 vs 程式實作落差修正」條目。
- ❌ **已排除（誤判，非真實 bug）**：`派遣間諜`（B3）——2026-07-11 覆核後確認實際生效的流程本來就是對的，原盤點依據的是死代碼，詳見上方段落，不需要處理。
- ✅ 已完成：**A2（部分）：`非暴力`**（民運派、性別革命）—— 2026-07-11 修正並驗證，詳見上方 A2 段落與 `TODO.md`「規則資料 vs 程式實作落差修正」條目。
- ✅ 已完成：**A2（部分）：`紅軍派系`**（改革開放派）—— 2026-07-11 實作完成（重用時代關卡的檢視重排機制），並順帶補上「陣營能力後端歸屬檢查」（先前任何玩家可透過 WebSocket 呼叫別家能力）；同時更正 `民族祭儀`「完全未實作」為誤判——runtime 與前端 UI 早已存在，缺的只是能力名稱對照（已補上）。詳見 A2 段落與 `TODO.md`。
- ✅ 已完成：**A2（收尾）：`民族祭儀`／`賭徒耳語` 三個文字偏差**（墊底手牌玩家選擇、沒猜中二選一、展示頂牌放回牌庫頂——③經使用者確認）—— 2026-07-11 修正並驗證。**A2 至此全部完成。**
- ⏳ 待處理：其餘全部項目（A1、A2 剩餘部分、A4、B1-c、B2 已無落差不需處理、B3 其餘項目、C1-C3）。
...
> **修正記錄（2026-07-11）**：45 個陣營全部完成資料轉換（`scripts/migrate_win_conditions.py`，往返檢查零失真）並接入 `victory.py`（新增 `required_any_of` 與 `宛地` scope）。驗證：`python3 scripts/validate/validate_text_faction_win_conditions.py`（7/7）；proof：`docs/records/rules-audit/TEXT_FACTION_WIN_CONDITIONS_VALIDATION_20260711.{json,md}`。
...
**額外資料缺口**：`dian_zhuang`（滇（壯）) 的 `win_condition_text` 本身就是佔位字串「待補（使用者提供清單未明列）」——**2026-07-11 已解決**：經比對與 `zhuang`（壯）完全重複（同根據地、同能力），使用者裁決為重複條目並已刪除（61→60 陣營）；A1 資料轉換無阻塞，待補陣營數為 45。
...
| 能力 | 受影響陣營 | 效果文字 | 影響 |
|---|---|---|---|
| [已修正 2026-07-11] 【非暴力】 | `minyun`（民運派）、`gender_revolution`（性別革命） | 「禁止持有武裝類卡牌。」 | 這兩個陣營應該不能持有/打出武裝類卡牌，但因為能力沒被解析進 `_player_effective_abilities`，`_player_is_nonviolent()`（`game.py:2899`，實際擋人用資金/打出武裝卡的地方在 `game.py:2965`、`3920`、`4530`）永遠回傳 False，**這兩個陣營完全沒有被擋下打出武裝卡**。注意：其他陣營（如 `tibet_dharamsala`、`uyghur_munich`）的「非暴力」是走結構化 `abilities` 陣列直接定義（效果文字甚至用字不同：「禁止持有或裝備類卡牌」vs 這裡的「禁止持有武裝類卡牌」），那些陣營的限制是有效的；只有透過 `abilities_text` 字串宣告的這兩個陣營漏掉。**修正記錄（2026-07-11）**：在 `direct` 對照表補上「非暴力」定義，沿用既有 `_player_is_nonviolent()` 機制；已核對過 `data/raw/action_cards.csv` 全部46張卡的種類欄，確認這個遊戲的卡牌資料裡沒有「裝備」種類的卡，所以沿用既有實作（擋 `{armed,equipment}`）在目前資料下跟只擋「武裝類」效果一致，沒有範圍擴大的疑慮，不需要另外新增範圍更窄的限制。驗證：`python3 scripts/validate/validate_minyun_gender_revolution_nonviolent.py`（7/7 passed）；proof：`docs/records/faction-ui/MINYUN_GENDER_REVOLUTION_NONVIOLENT_FIX_VALIDATION_20260711.{json,md}`；`TODO.md` 已同步新增對應 `[done]` 條目。 |
| [已修正 2026-07-11] 【紅軍派系】 | `reform_opening`（改革開放派） | 「每回合可檢視1次牌庫頂3張牌，將其以任意順序放回牌庫頂，並抽1張牌。」 | 整個能力沒有實作，這個陣營少一個主動能力可用。**修正記錄（2026-07-11）**：補上名稱對照與 `_activated_faction_action` 分支，重用 `era_inspect_deck_top_and_reorder` 重排機制＋`draw_after_reorder` 旗標；前端加 `reform_opening` 發動按鈕；並補上非紅軍陣營能力的後端歸屬檢查。驗證：`python3 scripts/validate/validate_red_faction_inspect_reorder.py`（5/5）；proof：`docs/records/faction-ui/RED_FACTION_INSPECT_REORDER_VALIDATION_20260711.{json,md}`。 |
| [盤點誤判更正＋部分修正 2026-07-11] 【民族祭儀】 | 12 個陣營：`dian_zhuang, zhuang, yi, bai, hani, dai, miao, tujia, dong, buyei, yao, li`（雲貴少數民族類陣營） | 「在己方行動階段，可將1張手牌放進牌庫底，猜牌庫頂牌購買費用奇偶，並展示牌庫頂牌。若猜中可獲得2點宣傳與2點資金，沒猜中則獲得2點宣傳或2點資金。」 | **原盤點「完全沒有實作」是誤判**：runtime 分支（與 `賭徒耳語` 共用的猜奇偶邏輯，含猜中2+2/沒猜中2宣傳的獎勵差異）與前端 UI（12 陣營按鈕、猜奇偶 modal、結果顯示）早已存在，缺的只是 `_resolve_ability_text` 名稱對照（2026-07-11 已補上，配合新的後端歸屬檢查）。~~仍殘留三個與能力文字的偏差~~ **三個偏差已於 2026-07-11 全部修正**（③經使用者確認為「看完放回牌庫頂」）：①墊底手牌改由玩家選擇（單張自動）；②沒猜中改二選一（2宣傳/2資金）；③展示的頂牌放回牌庫頂——`賭徒耳語` 的①③一併修正。驗證：`python3 scripts/validate/validate_faction_action_guess_result.py`（7/7）。**A2 全部收尾。** |

**建議改善方向**：
1. 在 `_resolve_ability_text` 的 `direct` 字典補上這 3 個能力的定義（或依效果性質建立新的 `ability_templates` ref，因為「民族祭儀」是 12 個陣營共用的，適合當作模板而非個別陣營硬編碼）。**「非暴力」已於 2026-07-11 完成**，詳見上表。
2. ~~「非暴力」尤其要小心……~~——已確認資料裡沒有「裝備」種類卡牌，沿用既有實作沒有範圍擴大疑慮，見上表修正記錄。
3. ~~「民族祭儀」建議直接參考 `賭徒耳語` 的猜奇偶程式碼結構改寫~~——覆核後發現該分支早已存在（原盤點誤判）；**`紅軍派系` 已於 2026-07-11 完成**；`民族祭儀` 剩餘工作改為修正上表列出的三個文字偏差（①②為明確缺口、③待確認）。
...
**修正記錄（2026-07-10）**：已採用短期修法，在 `_factions_sharing_with` 補上 `'香港' in text → shared.update(['hong_kong'])`。驗證：`python3 scripts/validate/validate_yue_aomen_hongkong_shared_org.py`（5/5 passed，涵蓋兩個新修正方向、兩個既有方向回歸測試、一個無關陣營 sanity check）；proof：`docs/records/faction-ui/YUE_AOMEN_HONGKONG_SHARED_ORG_FIX_VALIDATION_20260710.{json,md}`；`TODO.md` 已同步新增對應 `[done]` 條目。中期建議（把全部共用組織關係轉成結構化欄位）尚未執行，維持待處理。

---

### A4. [已裁決並實作 2026-07-11] `rules.md`「共同勝利：達成2/3條件」在目前資料下缺乏明確對應
...
- **[已修正 2026-07-10] (B1-a)『東洋奧援』的中介資料 `data/cards/support_taxonomy.v1.1.json` 本身已損毀/過期，與它自己的產生腳本重新產生的結果不一致**：目前檔案裡「東洋奧援」的其中一個地區變體只保留了「本地區（東洋）」的 III 級條件、缺少 II 級條件；另一個變體只保留「臺灣、南洋」這組地區搭配，CSV 規定的第二組地區搭配「北國、英美」完全遺失。實際影響：玩家若同時主導北國＋英美（但不主導東洋/臺灣/南洋），應該要能達到 II 級（「在牆內於己方組織1格內建立1個組織」）卻會被卡在 I 級（僅獲得2點宣傳）。用現有產生器腳本重新根據 `support_cards.v1.1.json` 跑一次可以重現正確的4地區結構，證實這是資料檔損毀／未同步，不是刻意設計。**建議改善**：重新執行 `scripts/build_support_card_taxonomy.py` 產生正確的 `support_taxonomy.v1.1.json` 並 commit，同時建議之後把「產生器輸出是否與來源資料同步」做成一個可重複執行的驗證腳本（避免下次來源資料更新又忘記重新產生）。
  ...
  **修正記錄（2026-07-10）**：已重新執行 `scripts/build_support_card_taxonomy.py` 重新產生 `data/cards/support_taxonomy.v1.1.json`（補回「北國、英美」配對）。修正資料後，另外發現並一併修正了一個原本被資料損毀遮蔽、從未在正常遊戲中觸發過的程式碼 bug：`server/game.py:_resolve_support_card_effect` 對 `東洋奧援` tier2 有一段 `region_index == 0` 特例，會誤回傳跟 tier3 相同的「無視距離建立」效果，而不是 tier2 該有的「1格內建立」；因為資料損毀時 index0 幾乎不可能真的走到 tier2 分支，所以這段錯誤程式碼直到資料修正後才會被觸發（此時 index0 變成「臺灣、南洋」配對）。已移除該特例分支，統一回傳 `interactive_build_near_inner`。驗證：`python3 scripts/validate/validate_east_asia_support_taxonomy_fix.py`（4/4 passed）；proof：`docs/records/support-cards/EAST_ASIA_SUPPORT_TAXONOMY_FIX_VALIDATION_20260710.{json,md}`；`TODO.md` 已同步新增對應 `[done]` 條目。

- **[已修正 2026-07-11] (B1-b)『南洋奧援』I 級「抽1張牌，再從所有手牌中棄掉1張牌」的棄牌沒有給玩家選擇權，效果等同沒抽沒棄**：實作是直接 `player.hand.pop()`，而抽到的牌是被 append 到手牌尾端，所以 `pop()` 彈出的正好就是剛抽到的那張牌——等於「抽1張又立刻棄掉同一張」，淨效果是 no-op，跟卡面文字暗示的「玩家可以從所有手牌中任選1張棄掉」（也就是抽到的新牌可能被留下、棄掉舊手牌）不符。**建議改善**：改成先抽牌加入手牌，再讓玩家（或依現有其他棄牌選擇的既有 UI 模式）從全部手牌中任選1張棄掉，而不是寫死彈出最後一張。

  **修正記錄（2026-07-11）**：改成用既有的 `_set_pending_card_choice()` 對整副手牌開一個真正的棄牌選擇（新 choice_key `draw_then_discard_choice`），`_resolve_card_choice()` 補上對應解析（移到棄牌堆，不是移除出局）。驗證：`python3 scripts/validate/validate_south_seas_support_tier1_discard_choice.py`（2/2 case，證明可以選擇棄掉「別的」那張、保留剛抽到的牌）；同步更新 `scripts/validate/validate_support_card_effects_runtime.py` 讓既有回歸測試改成先解決棄牌選擇再檢查最終狀態（12/12 case 全過）。proof：`docs/records/support-cards/SOUTH_SEAS_SUPPORT_TIER1_DISCARD_CHOICE_VALIDATION_20260711.{json,md}`；`TODO.md` 已同步新增對應 `[done]` 條目。
...
- **[已修正 2026-07-10]『樹立信心』：條件判定看錯欄位，跟已追蹤的 `點燃熱情` 是同一種 root cause，但這張卡本身沒被列進 TODO.md**。`conditional_draw` 的 `played_money_card` 判定條件（`game.py:4001-4002`）目前是檢查該回合打出的牌「種類（`card_type`）是否等於 `money`」，而不是檢查「購買費用是否包含資金成分」。例如 `謀劃`／`高效行動`／`誘導虛耗` 種類是「指揮」但購買費用含資金，玩家打出這些牌後應該要能讓 `樹立信心` 額外抽牌，實際上不會觸發。

  **修正記錄（2026-07-10）**：跟 `點燃熱情` 一起修正，且順帶把同一根因驅動的4個陣營能力（`商貿組織`、`基金會`/`共合會`、`民族調和`/`星星之火`、`人同此心`）也一起修正——全部改成用 `_card_purchase_cost()` 判斷實際購買費用組成，取代 `effective_type`（種類）判斷；維持奧援卡不算入的既有行為。修正過程中額外抓到並修好一個因此浮現的新問題：`點燃熱情`/`樹立信心`的「其它」牌語意（改看費用後，兩張卡本身的購買費用剛好都含資金/宣傳，會誤觸發到自己），解法是改記錄「打出前」的旗標快照供 `conditional_draw` 讀取；也順手補上反應延遲流程（`_resume_reaction_pending_action`）先前完全沒有設定這兩個旗標的既有缺口。驗證：`python3 scripts/validate/validate_cost_composition_triggers.py`（9/9 case 全過）；proof：`docs/records/action-cards/COST_COMPOSITION_TRIGGERS_FIX_VALIDATION_20260710.{json,md}`；`TODO.md` 已同步新增對應 `[done]` 條目（同一條目也涵蓋已追蹤的 `點燃熱情`）。

- **[覆核後判定為非問題，2026-07-11]『派遣間諜』：原盤點誤判**。原始盤點依 `effect_engine.py:543-572`（`dissolve` etype 帶 `requires_self_sacrifice`）描述「犧牲己方組織與尋找對手目標順序顛倒」，但**實際追蹤 `server/game.py:play_card()` 的分派邏輯後發現，`派遣間諜`／`內應間諜` 在 `play_card()` 裡有專屬分支（約 4060-4076 行），一律呼叫 `_start_card_dissolve_interaction()` 走互動式兩步選擇流程，根本不會進入 `action_engine.execute()`、也就不會走到 `effect_engine.py` 那段 `dissolve` etype**——原盤點指出的那段程式碼對這張卡來說是永遠不會被執行到的死代碼。實際生效的流程（`_start_card_dissolve_interaction` → `_interactive_support_sacrifice_towns`，`game.py:2200-2219`；選定犧牲城鎮後的結算，`game.py:2381-2408`）本身邏輯是對的：第一步只列出「本來就在敵方組織範圍內」的己方城鎮供犧牲（`_interactive_support_sacrifice_towns` 內部已經先用 `_interactive_support_dissolve_targets_near_town` 過濾過，沒有合法目標的城鎮根本不會被列為犧牲選項）；第二步玩家確認要瓦解的敵方目標之後，才真正扣掉犧牲城鎮的組織（`game.py:2393-2395`），順序是「先確認有效目標、才真的犧牲」，不是反過來。已用一段實際跑 `play_card`／`resolve_pending_choice` 的手動驗證重現：玩家有兩個組織（一個在敵方1格內、一個很遠），系統只把「在敵方1格內」的那個列為可犧牲選項，選定後才真正扣組織、且瓦解目標正確對應到犧牲城鎮本身的範圍。**這項不需要修改程式，從待處理清單移除。**

- **[已修正 2026-07-10]『情報網』：非反應時機下，選項C（取消對方能力）仍然可以被選、但選了沒有任何效果**。`choose_one`（`effect_engine.py:399-419`）不管是不是在別人的反應時機被打出，永遠把三個選項都列出來，包含選項C「其他玩家行動時打出，取消1張對方所打出行動卡之能力」。如果玩家在自己回合正常打出這張卡並選了C，會呼叫 `cancel_card` 但因為不在真正的反應情境裡，只會記一筆「canceled unknown card」的日誌、沒有任何實際效果——玩家會以為選項有效但其實是空轉。真正的反應流程（`game.py:3860`）其實有正確跳過 `choose_one`、直接走 `cancel_card`，所以這個問題只發生在「正常回合內打出情報網」的路徑。應該要在非反應情境下直接不提供/停用選項C。

  **修正記錄（2026-07-10）**：確認選項C在 `choose_one` 這條路徑裡本來就永遠不會有正確結果（真正的反應取消完全走另一套獨立流程，從不經過 `choose_one`），所以直接把選項C從 `data/action_cards_structured.v1.1.json` 的 `情報網` `choose_one` 選項清單移除，而不是加條件判斷。驗證：`python3 scripts/validate/validate_intel_network_no_reaction_option_own_turn.py`（2/2 case、8 項 check 全過，含反應流程回歸測試）；proof：`docs/records/action-cards/INTEL_NETWORK_NO_REACTION_OPTION_OWN_TURN_VALIDATION_20260710.{json,md}`；`TODO.md` 已同步新增對應 `[done]` 條目。

- **[已全部修正：距離1格子句隨 S2 於 2026-07-11 完成；重複建立子句＋確認流程亦於 2026-07-11 完成]『組織經驗甲』：問題比 TODO.md 現有記錄的還深，缺了兩整段規則子句**。（重複建立驗證：`python3 scripts/validate/validate_org_exp_a_repeat_build.py` 5/5；proof `docs/records/action-cards/ORG_EXP_A_REPEAT_BUILD_VALIDATION_20260711.{json,md}`。）TODO.md 目前只記錄「缺少確認是否要多花4點以上卡牌」的 UI 流程問題；但實際上 `data/action_cards_structured.v1.1.json` 裡這張卡的結構化效果就只有 `[{"type":"build","range":"ignore_distance"}]` 一步，代表整段規則完全沒有程式對應：(1)「每從手上棄掉1張購買費用4點以上的牌，可重複上述動作1次」——完全沒有任何重複建立的迴圈/邏輯存在；(2)「無法無視距離建立牆內組織者，本牌於牆內建立組織距離為1格」——這是給「本來就不能無視距離建立」的陣營（例如 A2 提到的 `uyghur_munich` 的『新疆社會管控』限制）的例外規則，但 `_card_build_town_choices`（`game.py:1034`）目前只有 `ignore_distance`（全部城鎮皆可）跟一般固定範圍 BFS 兩種分支，沒有針對這張卡特別處理「距離改為1格」的例外。建議把 TODO.md 對應項目的範圍描述更新為涵蓋這兩段，而不只是確認流程。

- **[已修正 2026-07-11]『行動預告』/『行動募資』：本回合買了2張以上的牌時，玩家沒有選擇要把哪張放回牌庫頂，永遠自動選最近買的那張**。（修正記錄：買2張以上改開卡牌選擇、剩餘效果選後續跑；並連帶修掉回合結束提示流程效果 pending 時懸空選擇的隱患。驗證 `python3 scripts/validate/validate_topdeck_purchased_choice.py` 4/4；proof `docs/records/action-cards/TOPDECK_PURCHASED_CHOICE_VALIDATION_20260711.{json,md}`。）兩張卡共用同一個 `topdeck_purchased_this_turn` 實作（`effect_engine.py:422-430`），直接從 `turn_log['purchased_cards_this_turn']` 挑「最近一張還在棄牌堆的牌」，沒有讓玩家選。因為 `buy_card`（`game.py:4519`）本身沒有限制每回合只能買1張，所以「本回合買了2張以上」是完全合法會發生的情況，此時卡面文字「將本回合購得的**1張**牌置於牌庫頂」隱含的玩家選擇被跳過了。
...
- ~~`點燃熱情` 條件成立時應抽2張、實際只抽1張~~ ——**已於 2026-07-10 跟 `樹立信心` 一起修正**，詳見上方 B3『樹立信心』段落的「修正記錄」與 `TODO.md`。
...
3. **A2（3 個能力字串未解析，影響 15 個陣營）**——~~「非暴力」~~、~~「紅軍派系」~~ **均已於 2026-07-11 修正**；`民族祭儀` 經覆核為誤判（早已實作，名稱對照已補），剩餘工作是三個文字偏差（詳見 A2 段落表格）。
4. ~~**A3（粵/澳門↔香港共用組織單向生效）**~~ ——**已於 2026-07-10 修正**（短期字串 pattern 修法），詳見上方 A3 段落的「修正記錄」與 `TODO.md`；中期把全部共用組織關係轉成結構化欄位的建議仍待處理。
5. ~~**B3 中的 `派遣間諜`（犧牲組織後才找目標，可能白白損失組織卻沒瓦解到敵方）**~~ ——**2026-07-11 覆核後判定為誤判、非真實 bug**，實際生效的互動式流程邏輯是對的，原盤點依據的是一段永遠不會被執行到的死代碼，詳見上方段落。不需要修改，從清單移除。
6. ~~**B3 中的 `樹立信心`（條件判定看種類而非購買費用組成，跟已追蹤的 `點燃熱情` 同根因）**~~ ——**已於 2026-07-10 修正**（含 `點燃熱情` 與4個陣營能力），詳見上方段落的「修正記錄」與 `TODO.md`。
7. **B3 中的 `組織經驗甲`（TODO.md 現有描述需擴大範圍，兩段規則子句完全未實作）**——請一併更新 TODO.md 描述再排入既有 P1 流程處理。
8. ~~**B1-b（`南洋奧援` I 級棄牌等於沒抽沒棄）**~~ ——**已於 2026-07-11 修正**，詳見上方段落的「修正記錄」與 `TODO.md`。
9. ~~**B3 中的 `情報網`（非反應時機下選項C選了沒效果）**~~ ——**已於 2026-07-10 修正**，詳見上方段落的「修正記錄」與 `TODO.md`。
